...
def _to_openvins(cam: dict, imu: dict) -> dict:
    ...
    # OpenVINS T_imu_cam maps camera coordinates into the IMU frame, so it
    # is the inverse of Kalibr's T_cam_imu.
    ...
    return {
        "imu": {
            ...
            # Kalibr and OpenVINS use the same convention:
            # t_imu = t_cam + timeshift_cam_imu.
            "time_offset": cam["timeshift_cam_imu"],
        },
        "camera": {
...
def print_summary(cam: dict, imu: dict):
    ...
    print(f"相机-IMU 时间偏移: {cam['timeshift_cam_imu']*1000:.2f} ms (t_imu = t_cam + offset)")
    if cam['T_cam_imu']:
