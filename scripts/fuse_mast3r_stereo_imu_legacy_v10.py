...
def scale_consistency(
    ...
    return {
        ...
        "joint_scale_m_per_mast3r_unit": float(np.sqrt(imu_scale * stereo_scale)),
...
def align_relative_motion_positions(
    ...
    rotation, translation = rigid_align_positions(
    ...
    return aligned, valid, {
        "alignment": "SE3_relative_motion_to_visual_body_no_scale",
...
def refine_positions_full_rate_imu(
    ...
    visual_sigma_m = 0.0015 * (1.0 + 5.0 * activation)
    dynamic_sigma_mps2 = 0.75
...
def stereo_observation_confidence(
    ...
    bidirectional_error = float(
        observation.get("bidirectional_relative_disagreement", 0.0)
    )
    ...
    def build_system():
        ...
        for node in range(node_count):
            add_identity(
                ...
                np.sqrt(visual_prior_weights[node]) / visual_sigma_m,
            )
            row += 3
        ...
        add_identity(
            design,
            row,
            position_offset + 3 * (node_count - 1),
            1.0 / anchor_sigma_m,
        )
        row += 3
...
def run(args: argparse.Namespace) -> dict:
    ...
    stereo_report = json.loads(args.stereo_report.read_text(encoding="utf-8"))
    if stereo_report.get("result") != "PASS":
        raise ValueError("stereo scale report did not pass")
    ...
    for path in args.additional_stereo_report or []:
        ...
        if report.get("result") != "PASS":
            raise ValueError(f"additional stereo scale report did not pass: {path}")
    ...
    if args.imu_scale_report:
        imu_scale_report = json.loads(
            args.imu_scale_report.read_text(encoding="utf-8")
        )
        if imu_scale_report.get("result") != "PASS":
            raise ValueError("IMU scale report did not pass")
        ...
        metric_scale_quality = scale_consistency(
            float(imu_scale_report["scale"]),
            float(stereo_report["scale_m_per_mast3r_unit"]),
    ...
    if args.relative_motion_trajectory is not None:
        ...
        reference_times, reference_positions, _, _ = load_trajectory(
    ...
    if (
        ...
        and position_quality["position_correction_max_m"]
        > MAX_JOINT_POSITION_CORRECTION_M + 1e-9
    ):
        failures.append("joint_position_correction_too_large")
    if (
        position_mode in {"joint-inertial", "keyframe-graph"}
        and acceleration_quality["equation_rmse"]
