...
def select_docker2_scale_weight(
    graph_report: dict,
    docker2_to_mast3r_ratio: float,
    input_disagreement_p95_m: float,
    *,
    selected_weight: float = 0.475,
    max_stereo_imu_scale_disagreement: float = 0.03,
    min_cross_slam_scale_disagreement: float = 0.02,
    max_input_disagreement_p95_m: float = 0.05,
    max_stereo_edge_rmse_m: float = 0.004,
) -> tuple[float, dict]:
    """Enable a bounded second scale vote only when onboard checks agree."""
    if graph_report.get("schema") != "umi_mast3r_stereo_imu_fusion_v2":
        raise ValueError("invalid MASt3R graph report schema")
    if graph_report.get("result") != "PASS":
        raise ValueError("MASt3R graph report did not pass internal checks")
    if graph_report.get("slam_supervision") is not False:
        raise ValueError("MASt3R graph report must disable supervision")
    if graph_report.get("external_ground_truth_used") is not False:
        raise ValueError("MASt3R graph report must not use external ground truth")
    stereo_imu_disagreement = float(
        graph_report["metric_scale_consistency"]["relative_difference"]
    )
    stereo_rmse = float(
        graph_report["stereo_translation_fusion"]["stereo_edge_rmse_after_m"]
    )
    cross_slam_disagreement = abs(docker2_to_mast3r_ratio - 1.0) / (
        0.5 * (docker2_to_mast3r_ratio + 1.0)
    )
    values = (
        docker2_to_mast3r_ratio,
        input_disagreement_p95_m,
        selected_weight,
        stereo_imu_disagreement,
        stereo_rmse,
        cross_slam_disagreement,
    )
    if not all(math.isfinite(value) for value in values):
        raise ValueError("automatic scale-selection metrics must be finite")
    if docker2_to_mast3r_ratio <= 0.0:
        raise ValueError("cross-SLAM scale ratio must be positive")
    if not 0.0 <= selected_weight <= 1.0:
        raise ValueError("selected automatic scale weight must lie in [0, 1]")

    stereo_imu_consistent = (
        stereo_imu_disagreement <= max_stereo_imu_scale_disagreement
    )
    second_vote_is_informative = (
        cross_slam_disagreement >= min_cross_slam_scale_disagreement
    )
    trajectory_shapes_are_compatible = (
        input_disagreement_p95_m <= max_input_disagreement_p95_m
    )
    stereo_geometry_is_reliable = stereo_rmse <= max_stereo_edge_rmse_m
    accepted = (
        stereo_imu_consistent
        and second_vote_is_informative
        and trajectory_shapes_are_compatible
        and stereo_geometry_is_reliable
    )
    return (selected_weight if accepted else 0.0), {
        "enabled": accepted,
        "selected_weight": selected_weight if accepted else 0.0,
        "candidate_weight": selected_weight,
        "stereo_imu_scale_relative_difference": stereo_imu_disagreement,
        "maximum_stereo_imu_scale_relative_difference": (
            max_stereo_imu_scale_disagreement
        ),
        "cross_slam_scale_relative_difference": cross_slam_disagreement,
        "minimum_cross_slam_scale_relative_difference": (
            min_cross_slam_scale_disagreement
        ),
        "input_disagreement_p95_m": input_disagreement_p95_m,
        ...
        "policy": (
            "use a bounded Docker2 scale vote only when stereo/IMU scale agrees, "
            "the second vote is informative, and both trajectory shapes remain "
            "compatible"
        ),
    }


def docker2_position_policy(
    report_path: Path | None, trajectory_path: Path
) -> dict:
    """Decide whether Docker2 translation is safe to inject.

    A run rejected only for a raw position jump can still provide calibrated
    IMU attitude and a robust global scale vote, but its local translation must
    not be copied into the fused trajectory.
    """
    if report_path is None:
        return {
            ...
            "source_result": None,
            "source_failures": [],
            "local_translation_allowed": True,
            "policy": "no_run_report_provided",
        }
    report = json.loads(report_path.read_text(encoding="utf-8"))
    if report.get("schema") != "umi_docker2_run_acceptance_v1":
        raise ValueError(f"invalid Docker2 run report schema: {report_path}")
    if report.get("slam_supervision") or report.get("external_ground_truth_used"):
        raise ValueError("Docker2 run report must not use external supervision")
    corrected = report.get("corrected_trajectory")
    if corrected is None or Path(corrected).resolve() != trajectory_path.resolve():
        raise ValueError("Docker2 run report does not match the input trajectory")
    failures = list(report.get("runtime_watchdog", {}).get("failures", []))
    result = report.get("result")
    if result == "PASS" and not failures:
        allowed = True
        policy = "accepted_run_local_translation_enabled"
    elif (
        result == "FAIL"
        and failures == ["raw_trajectory_jump"]
        and report.get("runtime_error") is None
    ):
        allowed = False
        policy = "raw_jump_local_translation_disabled_attitude_and_scale_retained"
    else:
        raise ValueError(
    ...
    return {
        ...
        "local_translation_allowed": allowed,
        "policy": policy,
    }
...
def complementary_positions(
    ...
    injection_limit_m = None
    injection_clipped_frames = 0
    if adaptive_local_weight:
        injection_limit_m = roughness_threshold_m
        injection_norm = np.linalg.norm(injected, axis=1)
        injection_scale = np.minimum(
            1.0,
            injection_limit_m / np.maximum(injection_norm, 1e-12),
        )
        injection_clipped_frames = int(np.count_nonzero(injection_scale < 1.0))
        injected *= injection_scale[:, None]
    ...
    return fused, {
        ...
        "injection_limit_mm": (
            injection_limit_m * 1000.0 if injection_limit_m is not None else None
        ),
        "injection_clipped_frames": injection_clipped_frames,
...
def run(args: argparse.Namespace) -> dict:
    ...
    auto_scale_selection = None
    docker2_scale_weight = args.docker2_scale_weight
    if args.auto_docker2_scale_weight:
        graph_report = json.loads(args.graph_report.read_text(encoding="utf-8"))
        preliminary_rotation, preliminary_translation = rigid_align(
            docker_positions, mast3r_positions
        )
        preliminary_docker = (
            preliminary_rotation @ docker_positions.T
        ).T + preliminary_translation
        preliminary_disagreement_p95_m = float(
            np.percentile(
                np.linalg.norm(preliminary_docker - mast3r_positions, axis=1), 95
            )
        )
        docker2_scale_weight, auto_scale_selection = select_docker2_scale_weight(
            graph_report,
            docker2_scale_ratio,
            preliminary_disagreement_p95_m,
        )
    scale_ratio, scale_disagreement = joint_log_scale_ratio(
        docker2_scale_ratio, docker2_scale_weight
    )
    scale_quality.update(
        {
            ...
            "docker2_scale_weight": docker2_scale_weight,
            "automatic_selection": auto_scale_selection,
        }
    )
    position_policy = docker2_position_policy(
        args.docker2_report, args.docker2
    )
    local_weight = (
        args.docker2_local_weight
        if position_policy["local_translation_allowed"]
        else 0.0
    )
    adaptive_local_weight = (
        args.adaptive_local_weight
        and position_policy["local_translation_allowed"]
    )
    ...
    report = {
        ...
        "docker2_position_policy": position_policy,
...
def main() -> int:
    ...
    parser.add_argument("--docker2-report", type=Path)
    ...
    parser.add_argument("--auto-docker2-scale-weight", action="store_true")
    ...
    if args.auto_docker2_scale_weight and args.graph_report is None:
        parser.error("--auto-docker2-scale-weight requires --graph-report")
