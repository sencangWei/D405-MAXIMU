...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="D405 720p三路(RGB+Depth+左IR)＋外置IMU采集")
...
def preview_mosaic(frames) -> np.ndarray:
    ...
    depth_color = np.asanyarray(rs.colorizer().colorize(frames.get_depth_frame()).get_data())
...
def main() -> int:
    ...
    session = args.output_root.resolve() / f"d405_720p_all_{stamp}"
    ...
    bag_path = session / "d405_720p_all_streams.db3"
    ...
    try:
        ...
        print("[全流采集] 1280x720@30: 彩色YUYV + 左IR + 右IR + Depth；IMU 400Hz")
        ...
        with frame_csv_path.open("w", newline="", encoding="utf-8") as fp:
            ...
            while True:
                ...
                if not args.no_preview:
                    cv2.imshow("D405 720p: RGB | Depth | IR Left", preview_mosaic(frames))
