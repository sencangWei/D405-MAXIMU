...
OFFLINE_VINS_EXECUTABLE="$LOOP_WS/build/vins_fusion_ros2/vins_fusion_ros2_node"
OFFLINE_VINS_LIBRARY="$LOOP_WS/build/vins_fusion_ros2/vins/libvins_lib.so"
...
for artifact in "$VINS_EXECUTABLE" "$VINS_LIBRARY" "$OFFLINE_VINS_EXECUTABLE" \
  "$OFFLINE_VINS_LIBRARY" "$LOOP_EXECUTABLE" "$REPLAY_EXECUTABLE"; do
  if [[ ! -x "$artifact" ]]; then
...
HASH_MANIFEST="$BUILD_ROOT/product_live_hashes.env"
{
  ...
  printf 'PRODUCT_OFFLINE_VINS_SHA256=%s\n' "$(sha256sum "$OFFLINE_VINS_EXECUTABLE" | awk '{print $1}')"
  printf 'PRODUCT_OFFLINE_VINS_LIBRARY_SHA256=%s\n' "$(sha256sum "$OFFLINE_VINS_LIBRARY" | awk '{print $1}')"
...
echo "PASS：实时15 Hz与离线30 Hz两套产品产物构建完成。"
