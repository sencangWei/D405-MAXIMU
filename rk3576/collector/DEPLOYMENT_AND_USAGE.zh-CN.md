...
- 0.3.0 的抢救会因为「码流 AU 数少于帧索引行数」而直接拒绝。这在相机/码流故障导致的
  中断里很常见（采集器死在半帧上），但数据其实是可用的。
- 0.3.1 改为对账：发布的对数取「索引行数」与「码流实际 AU 数」的较小值，随录制发布的
  帧索引会同步截断到该值（索引不再声称视频里不存在的帧），manifest 如实记录每路实际
  帧数并加 `STREAM_LENGTH_MISMATCH` 告警。
- 0.3.3 修正删除的幂等判断：此前只要存在一条 `complete` 的删除记录，同一 session 的再次
  删除都会被当成重复请求直接返回，于是「删除孤儿之后才出现/残留的工作目录」永远删不掉
  （现场留下 3.1 GB 且列表可见却删不动）。现在只有在该 session 磁盘上已无任何残留时才
  视为重复请求；`incomplete-status` 也改为返回最近一条记录（原先总是优先返回抢救记录）。
- 0.3.2 补上一个缺口：抢救失败会在 `incoming/.recover-<session>/` 留下工作目录，孤儿被删
  之后就变成看不见也删不掉的垃圾（现场实测 3.1 GB）。现在这种目录会以
  `staging_leftover` 出现在「未完成录制」列表里并可删除；删除孤儿时也会一并回收它的工作
  目录。对只有工作目录、已无原始数据的条目，抢救会被拒绝（没有可抢救的采集）。
