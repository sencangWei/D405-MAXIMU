...
夹爪编码器是独立产品功能，不属于上面的五个 VIO 标定阶段，也不会改相机或 IMU
参数。设备装配和 STM32 固件身份与已验收配置一致时，运行：
...
终端显示原始角度、闭合/张开方向、开合百分比、单边行程、双边闭合距离及空载软垫
间距。手动操作不要求每次精确停到同一个角度；采集/数据集必须保存原始角度和
`closure_ratio`。毫米间距带约 `±1.5 mm` 不确定度，只适用于空载，不得把夹持受力时的
读数当作器械尺寸。

完全张开后若显示明显不在约 `66.9 mm`，或出现 `INVALID`、角度跳变、开合方向相反，
停止采集并检查磁铁、接线、STM32 固件和传感器诊断，不能用软件重置零点掩盖硬件位移。
