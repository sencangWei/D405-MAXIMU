...
历史 Mode B 版本已完成协议测试、PC 解析器测试、实板烧录以及 D405 + STM32 的
3 小时相机/IMU 传输压力测试，证据边界见 `release/HARDWARE_STATUS_20260823.md`。
本工作树新增的 AS5047P `ERRFL` 恢复版本目前只完成目标编译和离线测试；必须经 ST-Link
烧录后确认持续 `flags=3`，再做实板压力测试。上述历史证据不能替代新版本验收。
磁编码器角度到夹距的机械标定仍待完成，不能视为编码器产品验收。
