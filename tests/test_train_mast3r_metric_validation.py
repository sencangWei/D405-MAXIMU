...
def test_descriptor_vectors_only_preserves_shared_layers_and_confidence_rows():
    class LocalFeatures(torch.nn.Module):
        def __init__(self):
            super().__init__()
    ...
    model = train.configure_trainable_parameters(Model(), "descriptor-vectors-only")
    loss = sum(
        head.head_local_features.fc2.weight.sum()
        + head.head_local_features.fc2.bias.sum()
        for head in (model.downstream_head1, model.downstream_head2)
    ...
    assert not model.encoder.weight.requires_grad
    for head in (model.downstream_head1, model.downstream_head2):
        assert not head.head_local_features.fc1.weight.requires_grad
        weight = head.head_local_features.fc2.weight
        bias = head.head_local_features.fc2.bias
        assert weight.requires_grad and bias.requires_grad
        assert torch.all(weight.grad[:16] == 1)
...
def test_descriptor_vectors_only_requires_independent_confidence_channel():
    class Head:
        two_confs = False
    ...
    with pytest.raises(ValueError, match="independent descriptor confidence"):
        train.configure_trainable_parameters(Model(), "descriptor-vectors-only")


def test_descriptor_vectors_only_disables_weight_decay_for_masked_rows():
    assert train.training_weight_decay("descriptor-vectors-only") == 0.0
