...
struct ZeroVelocityFactor {
  ...
  template <typename T>
  bool operator()(const T *current_speed_bias, T *residuals) const {
    for (int i = 0; i < 3; i++) {
      residuals[i] = T(inv_velocity_sigma) * current_speed_bias[i];
    }
    return true;
  }
...
void Estimator::resetState() {
  ...
  stationaryAnchorPosition.setZero();
...
void Estimator::resetInitializationState() {
  openExEstimation = 0;
  isFirstPoseInitialized = false;
  isFirstIMUReceived = false;
  backCount = 0;
  frontCount = 0;
  frameCount = 0;
  solver_flag = SolverState::INITIAL;
...
void Estimator::processMeasurements() {
  double window_total_sum_ms = 0.0;
  double window_total_max_ms = 0.0;
  double window_solve_sum_ms = 0.0;
  double window_solve_max_ms = 0.0;
  double window_marginalize_sum_ms = 0.0;
  double window_marginalize_max_ms = 0.0;
  size_t window_queue_max = 0;
  while (isRunning.load()) {
    TicToc backend_time;
    TimestampedFeatureFrame feature;
    vector<IMUData> imu_datas;
    uint64_t frame_imu_samples = 0;
    double frame_gyro_sum = 0.0;
    double frame_gyro_sq_sum = 0.0;
    double frame_gyro_max = 0.0;
    double frame_accel_sum = 0.0;
    double frame_accel_sq_sum = 0.0;
    std::vector<double> frame_flow_norms;
    std::vector<double> frame_flow_x;
    std::vector<double> frame_flow_y;
    frame_flow_norms.reserve(400);
    frame_flow_x.reserve(400);
    frame_flow_y.reserve(400);
    ...
    if (options->hasImu()) {
      ...
      for (const auto &imu : imu_datas) {
        const double gyro_norm = imu.angular_velocity.norm();
        const double accel_norm = imu.linear_acceleration.norm();
        frame_gyro_sum += gyro_norm;
        frame_gyro_sq_sum += gyro_norm * gyro_norm;
        frame_gyro_max = std::max(frame_gyro_max, gyro_norm);
        frame_accel_sum += accel_norm;
        frame_accel_sq_sum += accel_norm * accel_norm;
        frame_imu_samples++;
      }
    ...
    for (const auto &feature_observations : feature.second) {
      for (const auto &observation : feature_observations.second) {
        ...
        const double flow_norm = flow.norm();
        if (std::isfinite(flow_norm)) {
          frame_flow_norms.push_back(flow_norm);
          frame_flow_x.push_back(flow.x());
          frame_flow_y.push_back(flow.y());
        }
      }
    }
    const double frame_imu_count = static_cast<double>(frame_imu_samples);
    const double frame_gyro_mean =
        frame_imu_samples > 0 ? frame_gyro_sum / frame_imu_count : 0.0;
    const double frame_gyro_variance =
        frame_imu_samples > 0
            ? std::max(0.0, frame_gyro_sq_sum / frame_imu_count -
                                frame_gyro_mean * frame_gyro_mean)
            : 0.0;
    const double frame_accel_mean =
        frame_imu_samples > 0 ? frame_accel_sum / frame_imu_count : 0.0;
    const double frame_accel_variance =
        frame_imu_samples > 0
            ? std::max(0.0, frame_accel_sq_sum / frame_imu_count -
                                frame_accel_mean * frame_accel_mean)
            : 0.0;
    double frame_flow_median = 0.0;
    double frame_flow_p90 = 0.0;
    double frame_flow_coherence = 0.0;
    if (!frame_flow_norms.empty()) {
      const size_t median_index = frame_flow_norms.size() / 2;
      std::nth_element(frame_flow_norms.begin(),
                       frame_flow_norms.begin() + median_index,
                       frame_flow_norms.end());
      frame_flow_median = frame_flow_norms[median_index];
      const size_t p90_index =
          static_cast<size_t>(0.9 * (frame_flow_norms.size() - 1));
      std::nth_element(frame_flow_norms.begin(),
                       frame_flow_norms.begin() + p90_index,
                       frame_flow_norms.end());
      frame_flow_p90 = frame_flow_norms[p90_index];
      const auto median_component = [](std::vector<double> values) {
        const size_t index = values.size() / 2;
        std::nth_element(values.begin(), values.begin() + index, values.end());
        return values[index];
      };
      const double median_x = median_component(frame_flow_x);
      const double median_y = median_component(frame_flow_y);
      frame_flow_coherence =
          std::hypot(median_x, median_y) /
          std::max(frame_flow_median, 1e-9);
    }
    const bool was_zero_velocity_active = zeroVelocityActive;
    const bool inertial_stationary =
        frame_imu_samples >= 5 &&
    ...
    const bool motion_evidence =
        translational_inertial_motion_evidence || visual_motion_evidence;
    ...
    if (zeroVelocityActive != was_zero_velocity_active) {
      if (zeroVelocityActive) {
        stationaryAnchorPosition = estimator_state[WINDOW_SIZE].position;
      }
      VINS_INFO << "[ZUPT] "
                << (zeroVelocityActive ? "activated" : "deactivated")
                << " gyro_mean=" << frame_gyro_mean
                << " gyro_max=" << frame_gyro_max
                << " accel_std=" << std::sqrt(frame_accel_variance)
                << " flow_median=" << frame_flow_median
                << " flow_p90=" << frame_flow_p90
                << " flow_coherence=" << frame_flow_coherence
                << " velocity="
                << estimator_state[WINDOW_SIZE].velocity.transpose();
    }
    const double imu_ms = imu_time.toc();
    {
      std::lock_guard<std::mutex> lock(processingMutex);
      TicToc image_time;
      ...
      const uint64_t processed = processedImageCount.fetch_add(1) + 1;
      const double total_ms = backend_time.toc();
      window_total_sum_ms += total_ms;
      window_total_max_ms = std::max(window_total_max_ms, total_ms);
      window_solve_sum_ms += lastSolveMs;
      window_solve_max_ms = std::max(window_solve_max_ms, lastSolveMs);
      window_marginalize_sum_ms += lastMarginalizeMs;
      window_marginalize_max_ms =
          std::max(window_marginalize_max_ms, lastMarginalizeMs);
      window_queue_max = std::max(window_queue_max, queue_depth);
      if (processed % 30 == 0) {
        VINS_INFO << "[PERF-BACKEND] processed=" << processed
                  << " queue=" << queue_depth
                  << " queue_max30=" << window_queue_max
                  << " imu_ms=" << imu_ms
                  << " image_ms=" << image_ms << " tri_ms="
                  << lastTriangulateMs << " optimize_ms=" << lastOptimizeMs
                  << " solve_ms=" << lastSolveMs
                  << " marginalize_ms=" << lastMarginalizeMs
                  << " outlier_ms=" << lastOutlierMs
                  << " slide_ms=" << lastSlideMs << " cloud_ms=" << cloud_ms
                  << " total_avg30_ms=" << window_total_sum_ms / 30.0
                  << " total_max30_ms=" << window_total_max_ms
                  << " solve_avg30_ms=" << window_solve_sum_ms / 30.0
                  << " solve_max30_ms=" << window_solve_max_ms
                  << " marginalize_avg30_ms="
                  ...
                  << estimator_state[WINDOW_SIZE].velocity.transpose()
                  << " accel_bias="
                  << estimator_state[WINDOW_SIZE].accel_bias.transpose()
                  << " flow_median=" << frame_flow_median
                  << " flow_p90=" << frame_flow_p90
                  << " flow_coherence=" << frame_flow_coherence
                  << " inertial_stationary=" << inertial_stationary
        ...
        window_solve_max_ms = 0.0;
        window_marginalize_sum_ms = 0.0;
        window_marginalize_max_ms = 0.0;
        window_queue_max = 0;
      }
    }
  }
  ...
  if (solver_flag == SolverState::NON_LINEAR &&
    ...
    if (!point_cloud.points.empty()) {
      ...
      keyframe.cloud = point_cloud;
      {
        ...
        if (keyframe_data_queue.size() >= kMaxQueuedKeyFrames) {
          keyframe_data_queue.pop();
          ++keyframe_data_drops;
          VINS_WARN << "[LOOP_KEYFRAME_QUEUE_DROP] total_drops="
                    << keyframe_data_drops.load();
        }
        keyframe_data_queue.push(std::move(keyframe));
      }
    }
...
void Estimator::printStatistics(Timestamp timestamp) {
  ...
  if (processedImageCount.load() % 30 != 0) return;
  VINS_INFO << "pos: " << estimator_state[WINDOW_SIZE].position.transpose()
            << " feat: " << featureManager.getFeatureCount()
            << " ba: " << estimator_state[WINDOW_SIZE].accel_bias.transpose()
            << " bg: " << estimator_state[WINDOW_SIZE].gyro_bias.transpose();
...
void Estimator::processInitialization(Timestamp timestamp) {
  ...
  if (options->isStereoWithImu()) {
    advance_frame = processStereoWithImuInitialization();
  }
...
bool Estimator::processStereoWithImuInitialization() {
  featureManager.initFramePoseByPnP(frameCount, estimator_state,
                                    cameraTranslation, cameraRotation);
  featureManager.triangulate(frameCount, estimator_state, cameraTranslation,
                             cameraRotation);

  if (frameCount != WINDOW_SIZE) return true;
  ...
  // [DIAG] 打印视觉 vs IMU 预积分 帧间旋转分歧 (solveGyroscopeBias 的输入)
  {
    auto it = all_image_frame.begin();
    while (std::next(it) != all_image_frame.end()) {
      auto jt = std::next(it);
      Eigen::Quaterniond q_ij(it->second.R.transpose() * jt->second.R);
      Eigen::Quaterniond dq(jt->second.pre_integration->delta_q);
      double v_ang = q_ij.toRotationMatrix().eulerAngles(0, 1, 2).norm();
      double i_ang = dq.toRotationMatrix().eulerAngles(0, 1, 2).norm();
      double diff =
          (q_ij.toRotationMatrix() - dq.toRotationMatrix()).norm();
      VINS_INFO << "[DIAG-ROT] frame pair visual_deg=" << v_ang * 57.3
                << " imu_deg=" << i_ang * 57.3
                << " diff_norm=" << diff;
      it = jt;
    }
  }
  Vector3d delta_bg = solveGyroscopeBias(all_image_frame, estimator_state);
  VINS_INFO << "[DIAG-BG] delta_bg = " << delta_bg.transpose();
  // 初始化守卫: 检查进入 NON_LINEAR 的*总* bg 而非增量. solveGyroscopeBias 内部
  // 已把 delta_bg 累加进 states[].gyro_bias. bg 过大说明初始化窗口被污染(移动物体/
  // 模糊/快速运动时视觉 PnP 旋转噪声被吸进 bg: 干净会话 0.0009-0.0015, 污染时 0.074),
  // 不进入 NON_LINEAR. 触发时完整清除初始化窗口和状态, 但保留尚未处理的输入队列;
  // 仅回滚 delta_bg 并滑窗会把污染的 PnP 位姿带到下一窗口, 下一次小增量可能误放行.
  // 阈值 0.01 rad/s (~0.57°/s): 本 IMU 干净会话 bg 仅 0.0009-0.0015 rad/s,
  // 污染窗口(移动手部占满画面)达 0.04-0.16 rad/s, 25 倍间隙. 0.05 会让 2.5°/s
  // 的脏窗口溜进 NON_LINEAR 再发散到 3000m (230503 skip_s=1.5: bg=0.043 通过守卫
  // -> 3665m). 0.01 保证只有真正干净的初始化才放行, 污染场景诚实报"初始化失败"
  // 而非输出垃圾轨迹.
  double bg_norm = estimator_state[0].gyro_bias.norm();
  if (bg_norm > 0.01) {
    VINS_WARN << "[INIT-GUARD] total gyro bias too large (" << bg_norm * 57.3
              << " deg/s), resetting initialization state.";
  ...
  optimize();
  double max_init_speed = 0.0;
  double max_init_displacement = 0.0;
  for (int i = 0; i <= WINDOW_SIZE; ++i) {
    max_init_speed =
        std::max(max_init_speed, estimator_state[i].velocity.norm());
    max_init_displacement = std::max(
        max_init_displacement,
        (estimator_state[i].position - estimator_state[0].position).norm());
  }
  VINS_INFO << "[INIT-QUALITY] current_speed="
            << estimator_state[WINDOW_SIZE].velocity.norm()
            << " max_speed=" << max_init_speed
            << " displacement=" << max_init_displacement
            << " features=" << featureManager.getFeatureCount()
            << " parallax_px=" << featureManager.last_average_parallax
            << " ba=" << estimator_state[WINDOW_SIZE].accel_bias.transpose()
            << " bg=" << estimator_state[WINDOW_SIZE].gyro_bias.transpose();
  updateLatestStates();
  solver_flag = SolverState::NON_LINEAR;
...
void Estimator::updateEstimates() {
  ...
  if (options->hasImu()) {
    ...
    if (zeroVelocityActive) {
      const Vector3d gauge_translation =
          stationaryAnchorPosition - estimator_state[frameCount].position;
      for (int i = 0; i <= frameCount; i++) {
        estimator_state[i].position += gauge_translation;
      }
      // Keep the state propagated into the next frame consistent with the
      // zero-velocity factor. Position anchoring alone can hide a growing
      // velocity until ZUPT exits, at which point it appears as a large jump.
      estimator_state[frameCount].velocity.setZero();
    }
  } else {
    for (int i = 0; i <= WINDOW_SIZE; i++) {
      estimator_state[i].rotation =
  ...
  if (options->hasImu()) {
    options->time_delay = para_Td[0][0];
    if (processedImageCount.load() % 30 == 0) {
      printf("[DIAG-TD] estimated td = %.6f s\n", para_Td[0][0]);
    }
  }
}
...
void Estimator::AddZeroVelocityFactor(ceres::Problem &problem) {
  if (!zeroVelocityActive || !options->shouldUseZupt() || !options->hasImu() ||
      frameCount < 1)
    return;

  auto *factor = new ceres::AutoDiffCostFunction<ZeroVelocityFactor, 3,
                                                 SIZE_SPEEDBIAS>(
      new ZeroVelocityFactor(0.0001));
  problem.AddResidualBlock(factor, nullptr, speedBiasArray[frameCount]);
}
...
void Estimator::optimize() {
  ...
  stage_time.tic();
  solveOptimization();
  lastSolveMs = stage_time.toc();
  updateEstimates();
  lastMarginalizeMs = 0.0;
  if (frameCount < WINDOW_SIZE) return;
  stage_time.tic();
  ...
  lastMarginalizeMs = stage_time.toc();
}
...
bool Estimator::getKeyFrameData(KeyFrameData &data) {
  std::lock_guard<std::mutex> lock(keyframe_data_mutex);
  if (keyframe_data_queue.empty()) {
    return false;
  }
  data = std::move(keyframe_data_queue.front());
  keyframe_data_queue.pop();
  return true;
}

uint64_t Estimator::getKeyFrameQueueDrops() const {
  return keyframe_data_drops.load();
}
...
void Estimator::fastPredictIMU(const IMUData &data) {
  ...
  fastPredictIMUWithoutLock(data);
}

void Estimator::fastPredictIMUWithoutLock(const IMUData &data) {
...
void Estimator::updateLatestStates() {
  // Resetting the optimized state and replaying buffered IMU samples must be
  // atomic with respect to the live IMU callback. Otherwise a new sample can
  // be propagated between two old buffered samples, making the predictor jump
  // backwards and then report multi-second timestamp gaps.
  std::lock_guard<std::mutex> propagate_lock(propagateMutex);
  latest_state = estimator_state[frameCount];
  latest_Q = latest_state.rotation;
  // 后处理修复: 不把 latestImuData 时间戳重置为帧时间戳(那会导致重放缓冲时
  // 第一条 IMU 的 dt>1s 触发 "Abnormal IMU timestamp jump" -> 预积分跳过 -> 发散).
  // 用 previousImuData 的连续时间, 重放 dt 保持 2.5ms.
  ...
  while (!tmp_imu.empty()) {
    fastPredictIMUWithoutLock(tmp_imu.front());
    tmp_imu.pop();
  }
}
