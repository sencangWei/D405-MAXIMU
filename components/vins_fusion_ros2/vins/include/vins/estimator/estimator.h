...
class Estimator {
 public:
  ...
  bool getKeyFrameData(KeyFrameData &data);
  uint64_t getKeyFrameQueueDrops() const;
 ...
 private:
  ...
  bool processStereoWithImuInitialization();
 ...
 private:
  ...
  void AddZeroVelocityFactor(ceres::Problem &problem);
  ...
  double lastTriangulateMs = 0.0;
  double lastOptimizeMs = 0.0;
  double lastSolveMs = 0.0;
  double lastMarginalizeMs = 0.0;
  double lastOutlierMs = 0.0;
  double lastSlideMs = 0.0;

  int stationaryFrameCount = 0;
  int movingFrameCount = 0;
  bool zeroVelocityActive = false;
  Vector3d stationaryAnchorPosition = Vector3d::Zero();
  ...
  mutable std::mutex keyframe_data_mutex;
  std::queue<KeyFrameData> keyframe_data_queue;
  std::atomic<uint64_t> keyframe_data_drops{0};
