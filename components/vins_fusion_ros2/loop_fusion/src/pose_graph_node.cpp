    ...
    const vins_fusion_ros2::msg::LoopKeyFrame::ConstSharedPtr msg)
{
    ...
    {
        printf("[LOOP_INPUT_DROP] reason=out_of_order sequence=%lu expected=%lu\n",
    ...
    if (msg->sequence > expected_sequence)
    {
        ...
        printf("[LOOP_INPUT_DROP] reason=transport sequence=%lu expected=%lu "
...
int main(int argc, char **argv)
{
    ...
    printf("min_loop_spatial_support: %.6f%s\n", MIN_LOOP_SPATIAL_SUPPORT,
           MIN_LOOP_SPATIAL_SUPPORT > 0.0 ? " (enabled)" : " (disabled)");
