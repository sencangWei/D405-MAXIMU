...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    {
        TicToc tmp_t;
		const std::vector<int> loop_candidates =
			detectLoopCandidates(cur_kf, cur_kf->index);
		std::vector<int> ordered_candidates;
		if (pending_loop_index_ >= 0 &&
			cur_kf->index - pending_loop_current_index_ <= 10)
			ordered_candidates.push_back(pending_loop_index_);
		for (const int candidate_index : loop_candidates)
			if (std::find(
					ordered_candidates.begin(), ordered_candidates.end(),
					candidate_index) == ordered_candidates.end())
				ordered_candidates.push_back(candidate_index);
		for (const int candidate_index : ordered_candidates)
		{
	...
	if (loop_connection_found)
	{
		...
		if (old_kf != nullptr)
        {
			...
			const bool relative_pose_consistent =
				translation_disagreement < 0.08 && yaw_disagreement < 4.0;

			if (same_historical_keyframe && temporally_adjacent && relative_pose_consistent)
				pending_loop_count_++;
			else
			{
				if (same_historical_keyframe && temporally_adjacent && pending_loop_count_ > 0)
				{
			         ...
			         kMaxAcceptedCorrectionM)
			{
				...
				reset_pending_loop();
			         ...
			         kAcceptedLoopCooldownKeyframes)
			{
			...
			{
				const Vector3d fused_correction_t =
					medianTranslation(pending_loop_correction_translations_);
				const double fused_correction_yaw =
					medianAngle(pending_loop_correction_yaws_);
...
std::vector<int> PoseGraph::detectLoopCandidates(
