  ...
  while (isRunning.load()) {
    ...
    for (const auto &feature_observations : feature.second) {
      for (const auto &observation : feature_observations.second) {
        ...
        const double flow_norm = observation.second.tail<2>().norm();
    ...
    const bool inertial_motion_evidence =
        frame_imu_samples >= 5 &&
        (frame_gyro_mean > 0.02 || frame_gyro_max > 0.05 ||
         std::sqrt(frame_accel_variance) > 0.08 ||
         std::abs(frame_accel_mean - gravity.norm()) > 0.15);
    const bool visual_motion_evidence =
        frame_flow_norms.size() >= 20 &&
        (frame_flow_median > 0.010 || frame_flow_p90 > 0.08);
    const bool motion_evidence =
        inertial_motion_evidence || visual_motion_evidence;
