    ...
    if (!options->USE_GPU_ACC_FLOW) {
      ...
      if (hasPrediction) {
        ...
        cv::calcOpticalFlowPyrLK(
            prev_img, cur_img, prev_pts, cur_pts, status, err, cv::Size(31, 31),
            5,
      ...
      if (hasPrediction) {
        ...
        cv::Ptr<cv::cuda::SparsePyrLKOpticalFlow> d_pyrLK_sparse =
            cv::cuda::SparsePyrLKOpticalFlow::create(cv::Size(31, 31), 1, 30,
                                                     true);
    ...
    if (!options->USE_GPU) {
      if (n_max_cnt > 0) {
        ...
        cv::goodFeaturesToTrack(cur_img, n_pts,
                                ...
                                0.005, options->min_feature_distance, mask);
      } else {
        n_pts.clear();
      }
    }
