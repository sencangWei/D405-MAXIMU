...
#pragma omp parallel for num_threads(NUM_THREADS)
  ...
  Eigen::LDLT<Eigen::MatrixXd> ldlt(Amm);
  if (ldlt.info() == Eigen::Success && ldlt.vectorD().minCoeff() > eps) {
    Eigen::MatrixXd solved_Amr = ldlt.solve(Amr);
    Eigen::VectorXd solved_bmm = ldlt.solve(bmm);
    if (solved_Amr.allFinite() && solved_bmm.allFinite()) {
      A = Arr - Arm * solved_Amr;
      b = brr - Arm * solved_bmm;
      used_ldlt = true;
    }
  }
