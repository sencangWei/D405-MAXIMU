                   ...
                   vector<double> &_point_id, int _sequence)
{
    ...
    computeStereoTrackedPoints();
    computeStereoORBPoints();
    if(!DEBUG_IMAGE)
    {
        image.release();
        right_image.release();
...
void KeyFrame::computeStereoTrackedPoints()
{
    stereo_point_3d.assign(point_2d_uv.size(), cv::Point3f());
    stereo_point_valid.assign(point_2d_uv.size(), 0);
    if (image.empty() || right_image.empty() || point_2d_uv.empty() ||
        STEREO_BASELINE_M <= 0.0)
        return;

    vector<cv::Point2f> right_points;
    vector<uchar> forward_status;
    vector<float> forward_error;
    cv::calcOpticalFlowPyrLK(image, right_image, point_2d_uv, right_points,
                             forward_status, forward_error, cv::Size(21, 21), 3);
    vector<cv::Point2f> backward_points;
    vector<uchar> backward_status;
    vector<float> backward_error;
    cv::calcOpticalFlowPyrLK(right_image, image, right_points, backward_points,
                             backward_status, backward_error, cv::Size(21, 21), 3);

    const Matrix3d R_w_c = origin_vio_R * qic;
    const Vector3d T_w_c = origin_vio_T + origin_vio_R * tic;
    int valid_count = 0;
    for (size_t i = 0; i < point_2d_uv.size(); ++i)
    {
        if (!forward_status[i] || !backward_status[i] ||
            cv::norm(backward_points[i] - point_2d_uv[i]) > 0.75 ||
            std::abs(right_points[i].y - point_2d_uv[i].y) > 1.5)
            continue;
        Eigen::Vector3d left_ray, right_ray;
        m_camera->liftProjective(
            Eigen::Vector2d(point_2d_uv[i].x, point_2d_uv[i].y), left_ray);
        m_camera_right->liftProjective(
            Eigen::Vector2d(right_points[i].x, right_points[i].y), right_ray);
        const double left_x = left_ray.x() / left_ray.z();
        const double right_x = right_ray.x() / right_ray.z();
        const double disparity_normalized = left_x - right_x;
        if (!std::isfinite(disparity_normalized) || disparity_normalized <= 0.0)
            continue;
        const double depth = STEREO_BASELINE_M / disparity_normalized;
        if (!std::isfinite(depth) || depth < 0.25 || depth > 3.0)
            continue;
        const Vector3d point_c(left_x * depth,
                               left_ray.y() / left_ray.z() * depth, depth);
        const Vector3d point_w = R_w_c * point_c + T_w_c;
        stereo_point_3d[i] = cv::Point3f(
            static_cast<float>(point_w.x()), static_cast<float>(point_w.y()),
            static_cast<float>(point_w.z()));
        stereo_point_valid[i] = 1;
        valid_count++;
    }
    printf("[STEREO_TRACKED_POINTS] current=%d valid=%d total=%zu baseline_m=%.6f\n",
           index, valid_count, point_2d_uv.size(), STEREO_BASELINE_M);
}

void KeyFrame::computeStereoORBPoints()
{
    stereo_orb_point_3d.assign(orb_keypoints.size(), cv::Point3f());
    stereo_orb_point_valid.assign(orb_keypoints.size(), 0);
    if (orb_descriptors.empty() || right_orb_descriptors.empty() ||
        STEREO_BASELINE_M <= 0.0)
        return;

    cv::BFMatcher matcher(cv::NORM_HAMMING, false);
    vector<vector<cv::DMatch>> forward_matches;
    vector<vector<cv::DMatch>> reverse_matches;
    matcher.knnMatch(orb_descriptors, right_orb_descriptors, forward_matches, 2);
    matcher.knnMatch(right_orb_descriptors, orb_descriptors, reverse_matches, 2);
    vector<int> reverse_best(right_orb_keypoints.size(), -1);
    for (const auto &matches : reverse_matches)
    {
        if (matches.size() >= 2 && matches[0].distance < 0.8f * matches[1].distance)
            reverse_best[matches[0].queryIdx] = matches[0].trainIdx;
    }

    const Matrix3d R_w_c = origin_vio_R * qic;
    const Vector3d T_w_c = origin_vio_T + origin_vio_R * tic;
    int valid_count = 0;
    for (const auto &matches : forward_matches)
    {
        if (matches.size() < 2 || matches[0].distance >= 0.8f * matches[1].distance)
            continue;
        const int left_index = matches[0].queryIdx;
        const int right_index = matches[0].trainIdx;
        if (left_index < 0 || right_index < 0 ||
            right_index >= static_cast<int>(reverse_best.size()) ||
            reverse_best[right_index] != left_index)
            continue;
        const cv::Point2f &left_point = orb_keypoints[left_index].pt;
        const cv::Point2f &right_point = right_orb_keypoints[right_index].pt;
        if (std::abs(left_point.y - right_point.y) > 1.5)
            continue;

        Eigen::Vector3d left_ray, right_ray;
        m_camera->liftProjective(Eigen::Vector2d(left_point.x, left_point.y), left_ray);
        m_camera_right->liftProjective(Eigen::Vector2d(right_point.x, right_point.y), right_ray);
        const double left_x = left_ray.x() / left_ray.z();
        const double right_x = right_ray.x() / right_ray.z();
        const double disparity_normalized = left_x - right_x;
        if (!std::isfinite(disparity_normalized) || disparity_normalized <= 0.0)
        ...
        const Vector3d point_c(left_x * depth,
                               left_ray.y() / left_ray.z() * depth, depth);
        const Vector3d point_w = R_w_c * point_c + T_w_c;
        stereo_orb_point_3d[left_index] = cv::Point3f(
            static_cast<float>(point_w.x()), static_cast<float>(point_w.y()),
            static_cast<float>(point_w.z()));
        stereo_orb_point_valid[left_index] = 1;
        valid_count++;
    }
    printf("[STEREO_ORB_POINTS] current=%d valid=%d total=%zu baseline_m=%.6f\n",
           index, valid_count, orb_keypoints.size(), STEREO_BASELINE_M);
}
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
    ...
    const size_t tracked_point_count = orb_keypoints.size();
    TicToc t_match;
    #if 0
        if (DEBUG_IMAGE)
        {
    ...
    #endif
    if (orb_descriptors.empty() || old_kf->orb_descriptors.empty() ||
        stereo_orb_point_valid.size() != orb_keypoints.size())
    {
        printf("[AUTO_LOOP_REJECT] current=%d matched=%d reason=no_orb_descriptors\n",
               index, old_kf->index);
        return false;
    }
    ...
    vector<vector<cv::DMatch>> forward_matches;
    vector<vector<cv::DMatch>> reverse_matches;
    temporal_matcher.knnMatch(orb_descriptors, old_kf->orb_descriptors,
                              forward_matches, 2);
    temporal_matcher.knnMatch(old_kf->orb_descriptors, orb_descriptors,
                              reverse_matches, 2);
    vector<int> reverse_best(old_kf->orb_keypoints.size(), -1);
    for (const auto &matches : reverse_matches)
    {
    ...
    for (const auto &matches : forward_matches)
    {
        if (matches.size() < 2 || matches[0].distance >= 0.75f * matches[1].distance)
            continue;
        const int current_index = matches[0].queryIdx;
        const int old_index = matches[0].trainIdx;
        if (current_index < 0 || old_index < 0 ||
            current_index >= static_cast<int>(stereo_orb_point_valid.size()) ||
            old_index >= static_cast<int>(reverse_best.size()) ||
            reverse_best[old_index] != current_index ||
            !stereo_orb_point_valid[current_index])
            continue;
        const cv::Point2f current_point = orb_keypoints[current_index].pt;
        const cv::Point2f old_point = old_kf->orb_keypoints[old_index].pt;
        Eigen::Vector3d current_ray, old_ray;
        m_camera->liftProjective(
            Eigen::Vector2d(current_point.x, current_point.y), current_ray);
        m_camera->liftProjective(
            Eigen::Vector2d(old_point.x, old_point.y), old_ray);
        matched_2d_cur.push_back(current_point);
        matched_2d_cur_norm.emplace_back(
            current_ray.x() / current_ray.z(), current_ray.y() / current_ray.z());
        matched_2d_old.push_back(old_point);
        matched_2d_old_norm.emplace_back(
            old_ray.x() / old_ray.z(), old_ray.y() / old_ray.z());
        matched_3d.push_back(stereo_orb_point_3d[current_index]);
        matched_id.push_back(static_cast<double>(current_index));
    }
    ...
    */
    ...
    #endif
    Eigen::Vector3d PnP_T_old = Eigen::Vector3d::Constant(
        std::numeric_limits<double>::quiet_NaN());
    Eigen::Matrix3d PnP_R_old = Eigen::Matrix3d::Constant(
        std::numeric_limits<double>::quiet_NaN());
    ...
    constexpr double kMaximumPnpRmsePx = 2.0;
    constexpr double kMaximumPnpP95Px = 3.0;
    const double required_spatial_support =
        std::max(MIN_LOOP_SPATIAL_SUPPORT,
                 MINIMUM_STEREO_LOOP_SPATIAL_SUPPORT);
    if ((int)matched_2d_cur.size() > MIN_LOOP_NUM &&
        pnp_inlier_ratio >= MIN_LOOP_INLIER_RATIO &&
        spatial_support < required_spatial_support)
    {
    ...
    if ((int)matched_2d_cur.size() > MIN_LOOP_NUM &&
        pnp_inlier_ratio >= MIN_LOOP_INLIER_RATIO &&
        pnp_reprojection_rmse_px <= kMaximumPnpRmsePx &&
        pnp_reprojection_p95_px <= kMaximumPnpP95Px)
    {
        ...
        constexpr double kMaxHighPrecisionLoopRelativeTranslationM = 0.05;
        if (abs(relative_yaw) < 30.0 &&
            relative_t.norm() < kMaxHighPrecisionLoopRelativeTranslationM)
        {
    ...
    if (pnp_ran && PnP_T_old.allFinite() && PnP_R_old.allFinite())
    {
        relative_t = PnP_R_old.transpose() * (origin_vio_T - PnP_T_old);
        relative_yaw = Utility::normalizeAngle(Utility::R2ypr(origin_vio_R).x() - Utility::R2ypr(PnP_R_old).x());
