...
实体:
  ...
  world/{unit}/imu/accel_mps2/*      加速度计曲线(50Hz)
  world/{unit}/imu/gyro_rad_s/*      陀螺仪曲线(50Hz)
  stats                              帧率/丢帧/姿态统计文本
...
class RerunVisualizer:
    ...
    def __init__(
        ...
        self._last_imu_ts = {n: 0.0 for n in unit_names}
    ...
    def log_pose(self, unit: str, pose, cam_fx: float = 600.0, cam_fy: float = 600.0,
                 ...
                 img_width: int = 1280, img_height: int = 720):
    ...
    def log_imu(self, unit: str, sample, max_hz: float = 50.0):
        ...
        ts = float(sample.ts)
        due, sample_ts = _rate_due(ts, self._last_imu_ts[unit], max_hz)
        if not due:
            return
        ...
        values = {
            "imu/accel_mps2/x": float(sample.ax) * accel_scale,
            "imu/accel_mps2/y": float(sample.ay) * accel_scale,
            "imu/accel_mps2/z": float(sample.az) * accel_scale,
            "imu/gyro_rad_s/x": float(sample.gx) * gyro_scale,
            "imu/gyro_rad_s/y": float(sample.gy) * gyro_scale,
