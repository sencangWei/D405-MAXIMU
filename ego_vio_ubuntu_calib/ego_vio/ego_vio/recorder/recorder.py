...
目录布局:
  <out_dir>/<session>/<unit>/
      ...
      imu.bin
      camera_ts.csv    (idx, ts_mono, ts_wall)
      imu_ts.csv       (counter, ts_mono)
"""
...
IMU_RECORD_SIZE = 32   # 每条 IMU 记录: ts(8) + counter(4) + 7 floats(28) ... 见 _pack_imu
...
class UnitRecorder:
    ...
    def put_color(self, idx: int, ts_mono: float, color, ts_wall: float):
        if self._q.qsize() >= self._q.maxsize:
            self.dropped += 1
            return
        self._q.put(("color", idx, ts_mono, color, ts_wall))
            ...
            try:
                if kind == "color":
                    ...
                    if self._imu_fp:
                        ...
                        self._imu_fp.flush()
                    self._imu_csv_w.writerow([s.counter, f"{s.ts:.9f}", f"{time.time():.6f}"])
