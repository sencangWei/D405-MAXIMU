...
class Runtime:
    ...
    def setup(self, record: bool = True, visualize: bool = True):
        ...
        if visualize:
            ...
            if viz_names:
                ...
                self.viz = RerunVisualizer(viz_names, purge_sec=30.0, max_points=6000)
                ...
                if ov_socket_units:
                    ...
                    self._odom_receiver = OpenVINSOdomReceiver(
                        on_pose=lambda p, name=target_unit: self._on_ov_pose(name, p),
                ...
                if ov_ros2_units:
                    self._start_ros2_odom_subscriber(ov_ros2_units)
        ...
        for u_cfg in self.cfg.units:
            ...
            # openvins_ros2 后端: 额外加 StubVIO 给可视化(即时轨迹, 不等 OpenVINS 初始化)
            if u_cfg.vio.backend == "openvins_ros2" and u_cfg.role == "realtime_vio":
                ur.vio_viz = StubVIO(name=f"stub_{u_cfg.name}")

            self.units[u_cfg.name] = ur

    def _start_ros2_odom_subscriber(self, unit_names: list):
        """为 openvins_ros2 后端启动 ROS2 odom 订阅, 位姿直接给可视化。"""
        import sys
        ...
        class _OdomNode(Node):
            def __init__(self_, runtime):
                ...
                self_.create_subscription(Odometry, "/ov_msckf/odomimu", self_.on_odom, 10)
            ...
            def on_odom(self_, msg):
                ...
                for name in unit_names:
                    self_.runtime._on_ov_pose(name, pose)

        node = _OdomNode(self)
        spin_thread = threading.Thread(target=lambda: rclpy.spin(node), name="odom-spin", daemon=True)
        spin_thread.start()
        print("[runtime] ROS2 odom subscriber started → /ov_msckf/odomimu")

    _ov_pose_count = 0
    _ov_active = False
    def _on_ov_pose(self, unit: str, pose):
        self._ov_pose_count += 1
        if not self._ov_active:
            self._ov_active = True
            print(f"[runtime] OpenVINS 已初始化, 切换到 VIO 位姿")
        ...
        if self.viz:
            if self._ov_pose_count <= 3 or self._ov_pose_count % 100 == 0:
                print(f"[runtime] odom pose #{self._ov_pose_count}: t={pose.t} q={pose.q[:2]}...")
    ...
    def _on_imu(self, unit: str, s: ImuSample):
        ...
        if ur.vio:
            ur.vio.feed_imu(s)
        # StubVIO 给可视化即时位姿 (不等 OpenVINS 初始化)
    ...
    def _on_camera(self, unit: str, f: CameraFrame):
        ...
        if self.recorder:
            ...
            if f.color is not None:
                rec.put_color(f.frame_idx, f.ts, f.color, ts_wall)
    ...
    def _print_stats(self, stats: dict):
        ...
        for name, s in stats.items():
            line = f"[{name}] "
            if "rate_hz" in s:
                line += f"IMU {s['rate_hz']:.0f}Hz ok={s.get('imu_ok',0)} drop={s.get('dropped_frames',0)} bad={s.get('frames_bad',0)} jit={s.get('dt_jitter_ms',0):.1f}ms"
            if "cam_fps" in s:
