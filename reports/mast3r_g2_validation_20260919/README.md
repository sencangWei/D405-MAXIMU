...
## 17. tracker 真值链为什么跳变 —— 特征已定死, 触发条件仍未找到
...
`libsurvive_runtime_config.json`、`tracker_integrity.json`、`tracker.log`,
**没有 sweep 级的原始录制**, 无法离线重放解算。

> **本节含一次公开撤回。** 我先前的结论是"跳变全部发生在 tracker 自身转速最高的瞬间"
> (角速率分位中位 99.6%)。该结论**已被证伪并撤回**, 理由见 §17.4 —— 那个判据是循环论证。
> 下面 §17.1/17.2/17.5 是站得住的; §17.3 是被证伪的那条; §17.6 是修正后的特征刻画。
> **触发条件目前仍然未知**, 不要引用任何"已经查清机制"的说法。

### 17.1 标定代之间只改了基站位姿, 几何没有退化

把全部会话的 `libsurvive_runtime_config.json` 解析出来逐项对比
...
两代都是近反平行配置, **没有退化**。"v11 把基站标成了病态几何"这个假说**排除**。

另: 全库唯一 `unlock_count != 0` 的会话是 `20260917_232851_720p_loop1`(LH0 = 83), 但它的
两站位姿与同时段其它会话**一致到 1e-5**(差异只是浮点尾数)。`unlock_count` 是个计数器,
不是漂移 —— 与 §16.2 那次误判同源, 别再被它带偏。

### 17.2 IMU 裁判不可用

思路: `tracker.csv` 的姿态是 world←tracker, 相邻样本的相对旋转与同区间陀螺积分在同一个
体系里, 可以直接比; 陀螺独立于 Lighthouse, 看似理想裁判。去时间偏置后全序列中位残差
只有 0.024°(235329)/ 0.011°(233028), 说明 tracker 平时姿态解得极准。但**在跳变处失效**:

- 跳变多数落在 tracker 中高速段。1500 deg/s 下 **±2ms 时间偏置 = ±3° 假残差**, 与待判的
  6° 同量级; 235329 那次跳变的 IMU 读数随偏置选择在 0.055°→4.178° 之间摆动。
- **IMU 自己在这个区间吐垃圾**: 残差最大的 12 个样本, IMU 全部报 ~3300 deg/s 而 tracker
  只报 ~40 deg/s(近似饱和的固定值)。拿它当裁判会把 IMU 的故障算成 tracker 的故障。

**结论: IMU 不能用来自动判定 tracker 跳变。**

### 17.3 【已撤回】"跳变发生在转速最高瞬间" —— 循环论证

原判据: 每次跳变取该样本的角速率 `rate[k] = |R_k^-1 R_{k+1}| / dt`, 看它在本条 take 角速率
分布里的分位, 得"中位 99.6%, 88/96 在 90 分位以上"。**这个数字是假的。**

`rate[k]` 涵盖的正是**跨越跳变的那一步**。跳变本身就是一次姿态阶跃, 于是在 `dR/dt` 上必然
造出尖峰 —— 用跳变的产物去证明跳变, 同义反复。剔除跳变本身、只看**前后相邻步**
(`rate[k-1]`、`rate[k+1]`, 都不含跳变):

| 步 | 中位分位 | ≥90 分位 | ≥50 分位 |
|---|---|---|---|
| `rate[k]` 跨跳变(含跳变本身) | 99.7% | 90/96 | 96/96 |
| `rate[k-1]` 前一步(无跳变) | **80.5%** | 30/96 | 76/96 |
| `rate[k+1]` 后一步(无跳变) | **81.1%** | 33/96 | 81/96 |

再看**绝对量级**(各条 take 角速率中位 3.0, P95 35.9 deg/s):

- 邻步速率中位 **17.2 / 18.3 deg/s**, 只有该条 P95 的一半
- **85/96(前)与 80/96(后)的跳变发生在 < 50 deg/s**
- 邻步 > 100 deg/s 的只有 4/96 与 8/96

所以跳变时刻的转速只是**略高于典型值**(约 80 分位, 随机应得 50 分位), 绝对量级是中速,
**远不是"转速最高的瞬间"**。§17.3 原结论**撤回**。

(交叉核对: `tracker_jump_rate_audit.py` 把前后邻步合在一起、且按相机窗口掩码, 报
邻步分位中位 **73.1%**、绝对速率中位 **17.8 deg/s**、**165/192 落在 < 50 deg/s** ——
与上表同一结论。该脚本已改为一并打印这两个数, docstring 里也写明了本陷阱。)

### 17.4 跳变是"位置在跳", 不是"分支翻转"

把每次跳变的相对变换 `D = R_{k+1} R_k^-1`(世界系)分解开:

- **旋转极小**: 中位 **0.913°**, 73/96 < 2°, **96 次里没有一次超过 30°**
- **平移很大**: 中位 **7.01mm**, P95 26.34mm, 最大 52.80mm
- 其中 **25/96 是纯平移跳**(旋转 < 0.5°)

真正的"换分支"(两站互换 / 轴指派混淆 / 180° 翻转)会是一次**大旋转**; 这里姿态始终正确,
只有位置迈了一步并且不回位。**"分支切换"这个叫法是误导的** —— 门脚本的
`BRANCH_SWITCH_RATIO` 判的是"跳了不回", 不是"换了离散分支"。

再排除两种"位置为什么会错"的常见解释:

| 假说 | 判据 | 结果 |
|---|---|---|
| 解算用了陈旧的/错位的时间 | `implied_dt = 位移 / 跳变前中位速度` 应聚在采样周期 8ms | **0/96** 落在 5–15ms; 中位 60.1ms, 跨度 25–157ms ⇒ **排除** |
| 解算滞后于运动(沿运动方向拖尾) | 纯平移跳的位移方向应与运动方向同向(cos→1) | 中位 cos = **−0.253**, 17/25 为负 ⇒ **排除** |

位移方向与运动方向基本无关(略偏反向), 说明这是个**横向的空间错位**, 不是时间问题。

### 17.5 v10 与 v11 的差别是幅度, 不是频率
...
两代在 **6–9mm 处一刀切开**, 没有重叠。跳变在两代都发生, 差别是 **v11 下同样的失效造成的
位移大了一倍以上**, 于是越过了门的阈值, 才表现为"v11 代 12/12 REJECT"。

**注意这条对候选机制是个约束**: 两代基线只差 0.8%(1762→1748mm), 却对应 3 倍以上的位移差。
所以"位移幅度 ≈ 基座几何尺度"的简单解释**不成立**, 幅度更可能由解算的局部极小间距决定。
...
- `lighthouse_tracker_branch_gate.py`(3mm 绝对下限 + P95×3)比 integrity 好, 但仍只覆盖
  幅度大的那些。
- 全库扫描(56 条会话): 21 条有跳变, **共 96 次**。

### 17.7 还剩下什么线索

特征已经被钉死(位置跳、姿态不跳、不回位、方向与运动无关、幅度由标定代决定), 但
**触发条件没找到**。可用工具都已经用尽: 会话目录里没有 sweep 级原始数据、libsurvive 的
`SV_WARN`(含 `Disambiguator got lost`)被默认日志级别压掉没有落盘、IMU 在关键速率段不可用。

仍然活着的一条线索: `tracker.log` 末尾的 `MPFIT stats: seed runs 17 / 9002` ——
9002 次解算里有 **17 次需要重新播种**。**若某次重播种落进邻近的另一个局部极小**,
就正好产生"位置迈一步、姿态几乎不变、之后一直从新解继续"的现象, 且位移幅度由极小的间距
决定(与 §17.5 的几何约束一致)。**但日志只有总计数字, 没有时间戳, 对不上 96 次跳变,
目前无法验证。**

要往前走, 需要的是**逐样本的解算器诊断**(种子是否重启、观测到几个站/几条 sweep、
残差是多少) —— 这是一次采集侧的改动, 不是分析侧能补的。

**给重标定的直接建议**: 重标之后拿一次**含快转**的 take 跑
`tracker_jump_rate_audit.py` + 分支门, 看最大单步是否回到 v10 代的 3–6mm 量级。
**静止验收测不出这个问题**(§ v11 的静止 P95 只有 0.231mm) —— 它只测抖动。

### 17.8 归一化之后: 跳变倾向是**标定代本身的属性**, 相差 44 倍

§17.5 用的是"每条 take 的最大单步", 那受运动量影响。把跳变数按**行进距离**归一
(总路程 = 相邻位置差之和)之后, 结论比 §17.5 干净得多:

| 标定代(基站位姿指纹) | 时段 | 会话 | 跳变 | 总路程 | **每米跳变** |
|---|---|---|---|---|---|
| `?`(无配置) | 09-08 | 10 | 2 | 27.69 m | 0.072 |
| `69c93307` | 09-09 | 2 | 3 | 10.64 m | 0.282 |
| `fc90e4b7` | 09-10/11 | 7 | 25 | 37.53 m | 0.666 |
| `8db513d3` | 09-12/13 | 12 | 14 | 44.90 m | 0.312 |
| **`2ec7d977`** | **09-14/15 (v10)** | 13 | **2** | **79.42 m** | **0.025** |
| **`8e2fc3eb`** | **09-16/18 (v11)** | 12 | **50** | 45.80 m | **1.092** |

**v10 与 v11 相差 44 倍, 而且 v10 的总路程还更长** —— 所以"v11 跳得多"不是"v11 动得多"。

这条同时改写了 §17.5 的框架: 幅度差异(2.82–5.87 vs 8.90–19.60mm)只是**次要表现**,
主因是**每米跳变率**。也说明 §17.5 那个"6–9mm 一刀切开"只在 v10/v11 这一对上成立,
不是普遍规律 —— 更早的 `fc90e4b7` 代最大单步到 **52.80mm**。

**对重标定最重要的含义**: v10 代证明了 **0.025 次/米 是做得到的**。所以这**不是**
"Lighthouse 天生如此、重标也没用"; 标定质量本身就是主导因素, 一次好的重标定有可能
把跳变率拉回 v10 的水平。
...
| 假说 | 判据 | 结果 |
|---|---|---|
| 快转打崩解算 | 剔除跳变本身后看前后邻步速率 | 只到 ~73–81 分位, 中位 17.8 deg/s ⇒ **排除**(见 §17.3) |
| 时间错位/陈旧解 | `位移/速度` 应聚在采样周期 8ms | 0/96 落在 5–15ms ⇒ **排除** |
| 运动方向拖尾 | 纯平移跳方向应与运动同向 | cos 中位 −0.25 ⇒ **排除** |
| 离散分支翻转 | 跳变应含大旋转 | 旋转中位 0.91°, 无一超 30° ⇒ **排除**(见 §17.4) |
| 基站几何退化 | 基线/朝向夹角/张角 | 两代均近反平行 ⇒ **排除**(见 §17.1) |
| GDOP 病态 | 法方程条件数 | v10 3.63 vs v11 3.93, **只差 8%**, 对应 44 倍跳变率差 ⇒ **排除** |

即: 触发条件是**标定结果里的某个还没被测到的量**, 不在上述任何一维上。

### 17.9 一条线索: 标定解自己记录的基站位姿方差

把 v10 与 v11 的 `libsurvive_runtime_config.json` 做**全字段 diff**(之前只口头说过
"变的只有 pose 与 floor-offset", 没逐字段验过):

- **`fcal*` 全同**(`fcalphase`/`fcaltilt`/`fcalcurve`/`fcalgibpha`/`fcalgibmag`/
  `fcalogeephase`/`fcalogeemag`)—— 那是光塔的**出厂标定**, 装置自带, 重标定改不了它。
  所以"角度观测被系统性偏置"这条**不成立**。
- `poser`(MPFIT)、`disambiguator`(StateBased)、id、mode、`ootx-ignore-sync-error` 全同。
- 变的只有: `pose`(已知)、`floor-offset`(−0.1720 → −0.1504)、
  **`variance`**、`lighthouse0.accel`(x 分量 0.0 → −1.0)。

`variance` 是**标定解自己吐出的基站位姿不确定度**(6 分量: 位置 3 + 旋转 3):

| | LH0 六分量和 | LH1 六分量和 | LH0 位置 3 项 |
|---|---|---|---|
| v10 | 1.728e-04 | 7.082e-05 | 1.605e-05 |
| v11 | 5.850e-04 | 4.104e-04 | 6.205e-05 |

**LH0 涨 3.4 倍, LH1 涨 5.8 倍。** 按全部 5 个有配置的标定代看:

| 指纹 | 时段 | 每米跳变 | LH0 方差和 | LH1 方差和 |
|---|---|---|---|---|
| `69c93307` | 09-09 | 0.282 | 2.042e-03 | 3.288e-04 |
| `fc90e4b7` | 09-10/11 | 0.666 | 2.042e-03 | 3.288e-04 |
| `8db513d3` | 09-12/13 | 0.312 | 6.373e-04 | 1.098e-04 |
| **`2ec7d977`** | **09-14/15 (v10)** | **0.025** | **1.728e-04** | **7.082e-05** |
| **`8e2fc3eb`** | **09-16/18 (v11)** | **1.092** | **5.850e-04** | **4.104e-04** |

**v10 是方差最小的一代, 也是跳变率最低的一代**(LH1 相关系数 +0.82)。

> **⚠ 这只是线索, 不是结论, 别当答案用。**
> 1. **n=5, 不显著**(p ≈ 0.09)。LH0 只有 +0.12, 相关几乎全部由 LH1 一项扛着。
> 2. **MPFIT 根本不消费 `bsd->variance`** —— `poser_mpfit.c` 里只有 `sensor-variance*`
>    那几个配置项, 没有任何读基站位姿方差的地方; 消费它的是 `survive_kalman_lighthouses.c`
>    (只用于 `poser: KALMAN`)。所以它**不可能是机制**, 只能是"标定质量"的**代理指标**。
> 3. 因此它有用的地方是**验收**: 重标定后若 `variance` 回到 v10 的 1.7e-4 / 7.1e-5
>    量级, 大概率是好的; 若落在 v11 的 5.9e-4 / 4.1e-4, 就值得先别录数据。
>    —— 但这需要更多样本来确认, 目前只能说"值得盯"。

另外 `lighthouse0.accel` 的 x 分量 v10 是 `0.000`, v11 是 `-1.000`(LH1 两代都是 `-1.000`)。
含义未查清, 记录在此备查。

---

## 18. 给下一次重标定的验收标准（由 §17 反推，有数据支撑）

基站昨晚又被移动了 ⇒ 需要 v12。§17 的结果可以直接转成验收标准 —— 因为**同一批重标定
报告里就有一个量与跳变率同向**，而且它此前被忽略了。

### 18.1 v10 与 v11 的标定报告逐项对照

| | **v10**（跳变率 0.025 /m） | **v11**（跳变率 1.092 /m） | 倍数 |
|---|---|---|---|
| 标定输入规模 | **32 个稳定场景 / 1024 组测量**（3 段原始记录 run7+run8+run10） | **13 有效场景 / 最大测量数 436** | 输入**少一半以上** |
| 静止验收 位置 P95 | **0.0568 / 0.0667 mm** | **0.231 / 0.311 mm** | **噪声差 4 倍** |
| 静止验收 位姿跳变 | **0**（明确记 0） | 未报该字段 | — |
| 重复重放 | 基线差 `0 mm` / 相对几何差 `0 mm / 0 deg` | 两次离线复算差 `1.1e-13 mm`（确定性同样好） | — |
| 在线解 vs 确定性离线解 | 未报 | **`8.585 mm / 0.455 deg`** | — |

**三个量全部指向同一方向**，而且量级对得上：

- `variance`（标定解记录的基站位姿不确定度）：LH0 涨 3.4×，LH1 涨 5.8×（§17.9）
- 静止验收噪声：4×
- **在线解 vs 离线解的差 8.585mm，与跳变位移中位 7.01mm 同量级**（§17.4）

即：**v11 的标定是用一半不到的观测、定出了一个不确定度大数倍的基站位姿**；静止噪声大 4 倍、
在线解与离线解能差 8.6mm，运动时这个不确定度就以 mm 级位置跳的形式冒出来。

> **置信度要如实说**：标定质量三指标 → 跳变率 这条链是 **n=2 代**的一致方向，
> 不是已证明的相关。机制上讲得通（标定不确定度小 ⇒ 解算的局部极小浅 ⇒ 跳得少跳得近），
> 量级也对得上，但**样本只有两代**。下面的标准据此提出，**用 v12 自己验证**。

### 18.2 v12 的验收标准（建议）

**① 标定输入量对齐 v10，别只收一段**
v10 用 3 段原始记录联合求解（1024 组测量）；v11 只有 436。建议至少 3 段、目标 ≥1000 组测量。

**② 静止验收用绝对阈值，不只看"看着很小"**
v11 的 0.231mm 当时没被当成问题，但它比 v10 的 0.057mm 差 4 倍。

- 位置 P95 **≲ 0.08 mm**（v10 是 0.057/0.067）—— 落在 0.2mm 量级就别开始录数据
- 明确记录 **位姿跳变次数**（v10 记的是 0，v11 没记这个字段）
- 独立进程重启两次，位置中心差 ≲ 0.02mm（v10 = 0.0184）

**③ 记录"在线解 vs 确定性离线解差"**
v10 没报这个量，v11 是 8.585mm。建议作为**必测项**：它和跳变位移同量级，是标定质量最直接的
探针。目标应当显著小于 1mm。

**④ 看 `variance`**
两个基站的 6 分量和应回到 v10 的 `1.7e-4` / `7.1e-5` 量级；落在 v11 的 `5.9e-4` / `4.1e-4`
就值得先别录。

**⑤ 静止验收之外，必须跑一次含快转的运动 take**
**这是 v11 侥幸通过验收的原因**：静止测试只测抖动，对运动下的解算失效是盲的（v10 那次
静止 P95 只有 0.057mm 却仍有跳变率 0.025/m 的残余）。跑
`lighthouse_tracker_branch_gate.py` + `tracker_jump_rate_audit.py`，看**每米跳变率**
是否回到 v10 的 **~0.025** 量级。

> ⚠ 以上 ①③④⑤ 是**建议**，需要 v12 的数据来验证 ② 的阈值定得对不对。
> 不要把这些当成已验证的规程引用。
