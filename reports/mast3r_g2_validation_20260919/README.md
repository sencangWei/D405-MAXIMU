...
| [gt_segment_split.py](gt_segment_split.py) | **真值切段造假的量化** —— 整条对齐 vs 按分支分段对齐(见 §16.3) |
...
> **⚠ 口径更正(必读): 上面这三个数字评的是 VINS 单链, 不是融合链。**
> 三个目录的 `precision.json` 里 `estimate` 字段都指向
> `slam/vio_corrected_stream.csv`, 且这些目录下**只有 `slam/`, 没有 `mast3r/`、
> 没有 `fusion/`** —— 新 720p 数据**从未跑过 MASt3R 融合管线**。
> 因此表中 Sim(3) 尺度 0.999~1.001 说的是 **VINS 这条链**, 不能拿来夸融合链。
> (§15.2 的"融合链尺度是好的"一句据此作废。)
...
- **新录制跑了, 但精度数字不可用** —— 不是 SLAM 变差了, 是这批 take 的真值被 tracker
  分支跳变污染(9 条里 8 条)。
- Sim(3) 最优尺度都在 0.999~1.001 —— 但这是 **VINS 单链**的尺度(见 §15.1 口径更正),
  说明这几条 VINS 输出的全局尺度没问题; 这与 §12 里"VINS 全局尺度可低到 0.628"不矛盾,
  因为 §12 说的是**融合输入**那条 VINS, 采样口径不同。
- **要拿到可信的新精度数字, 只有两条路**:
...
## 16. 到底是数据问题还是算法问题 —— 判别实验

§15 只说了"新数据不可用"。本节回答**不可用的那一部分该记在谁头上**, 以及
**老数据的结论有没有被一起污染**。三个实验, 全部只读。

### 16.1 实验一: 分支跳变是"一直有"还是"新出现的"?

判据: 把**每个会话**的 tracker 都过一遍分支门, 按标定代分组。标定代从会话目录里的
`libsurvive_runtime_config.json` 反查(哈希对到 `lighthouse_recalibration_*/libsurvive_config_frozen_v*.json`)。

| 标定代 | 可用会话 | REJECT(有分支切换) | 干净率 | 备注 |
|---|---|---|---|---|
| **v10**(09-14/09-15, §3~§14 那 18 项的来源) | 13 | **2** | **11/13 = 85%** | 2 条空 take(0 行 tracker)已剔除 |
| **v11**(09-16 起, 含全部 loop1) | 12 | **12** | **0/12 = 0%** | 含 3 条**非 loop1** 的 09-16 会话 |

**决定性的在最后一行**: `20260916_115339/120105/120410_fusion_v11_01/02/03` 是
**v11 标定、但不是闭环运动**的三条, 切换次数 1 / 4 / **17**, 全部 REJECT。
⇒ **不是 loop1 的闭环运动导致的, 是标定代。**

### 16.2 中途排除的两个错误假设(记下来免得再走一遍)

1. **"闭环运动触发分支跳变"** —— 被 16.1 的 09-16 三条否掉。
2. **"运行时配置漂移了"** —— 看到 loop1 的灯塔位姿与 v10 差 22.4cm、与 v11 的哈希不符,
   一度判定"配置漂移"。**错**。逐行 diff 后: loop1 与 frozen v11 的差异只有
   浮点尾数(`0.176545536830` vs `0.176545530558`, 约 6e-9 m = 6 纳米)和
   `unlock_count` 83→0 的记账字段 —— **loop1 用的就是 v11 的位姿**。
   那 22.4cm 是 v10→v11 一次**正当的重标定**。

   背景(来自 `lighthouse_recalibration_world_v11_20260916_consensus/README.md`):
   **基站被碰撞过**, v10 的两站相对几何已失效(相对 v10 变了 **365.588 mm / 3.888°**),
   v11 是按新几何重标的正确结果, v10 只保留供历史数据复现。

### 16.3 实验二: 真值切段到底造了多少假?

用 [gt_segment_split.py](gt_segment_split.py): 同一估计、同一真值算两遍 ——
(a) 整条一次 SE(3) 对齐(= 官方口径); (b) 在切换时刻切段、**每段各自对齐**。
(b) 把"真值自己断成两截"从 ATE 里剥掉, 差额就是真值造的假。
切换时刻从 tracker 的 `host_monotonic_s` 经 provenance 的
`clock_mapping.epoch_minus_monotonic_s` 换算到真值时间域。

| 项 | 口径 | RMSE | P95 | **max** | 10mm内 | 姿态 |
|---|---|---|---|---|---|---|
| **老数据** `batch5/group1` sparse | (a) 整条 | 4.98 | 9.47 | **19.32** | 95.9% | 1.81° |
| | (b) 分段 | 4.24 | 7.79 | **17.03** | 99.0% | 1.90° |
| | **真值造的假** | +0.75 | +1.68 | **+2.30** | −3.1% | −0.08 |
| **老数据** `batch5/group1` tight | (a) 整条 | 4.91 | 10.24 | **18.54** | 94.8% | 1.76° |
| | (b) 分段 | 4.37 | 8.98 | **15.91** | 95.4% | 1.91° |
| | **真值造的假** | +0.54 | +1.26 | **+2.63** | −0.6% | −0.16 |
| **新数据** `233028`(VINS 单链) | (a) 整条 | 9.98 | 15.94 | **22.89** | 70.2% | 5.12° |
| | (b) 分段 | **4.40** | **8.70** | **17.75** | **97.1%** | 5.76° |
| | **真值造的假** | **+5.57** | +7.24 | **+5.13** | −26.9% | −0.64 |
| **新数据** `235329`(VINS 单链) | (a) 整条 | 10.18 | 20.09 | **24.00** | 71.1% | 5.03° |
| | (b) 分段 | **5.19** | **9.76** | **20.55** | **95.6%** | 16.68° |
| | **真值造的假** | **+4.99** | +10.33 | **+3.44** | −24.5% | −11.65 |

两条完全不同的结论, 取决于数据代:

- **新数据(loop1): 真值造假如是主项。** `233028` 报出来的 RMSE 9.98, 剥掉真值损伤后
  只剩 **4.40**; 10mm 内比例从 70.2% 回到 **97.1%**。也就是说**那个 FAIL 大半是尺子坏了**。
- **老数据(batch5/group1): 真值损伤很小。** 只值 +0.75 / +0.54 的 RMSE、+2.3 / +2.6 的
  max —— 占 19.32 的约 **13%**, 而且**剥掉之后依然 FAIL**(17.03 / 15.91 ≫ 门 10)。

### 16.4 实验三: 剥掉真值损伤后, 尖峰还在不在?

**这是"数据问题还是算法问题"的分水岭。** 上表 (b) 列 —— 即真值损伤已剥离后的**真实
算法误差**:

| | (b) 的 max | (b) 的姿态 | 门限 | 判定 |
|---|---|---|---|---|
| 老 `batch5/group1` sparse | 17.03 | 1.90° | 10 / 2.0° | **仍 FAIL(max)** |
| 老 `batch5/group1` tight | 15.91 | 1.91° | 10 / 2.0° | **仍 FAIL(max)** |
| 新 `233028` | 17.75 | 5.76° | 10 / 2.0° | **仍 FAIL(max, rot)** |
| 新 `235329` | 20.55 | 16.68° | 10 / 2.0° | **仍 FAIL(max, rot)** |

**四个个案里, 尖峰和姿态在剥离真值损伤后全部依然超门。** 而且 `233028` 的分段明细显示
尖峰**落在段内部**而不是接缝处:

```
段0:  566 点  rmse  5.15  max 17.75
段1:  380 点  rmse  5.80  max 14.41
段2:  795 点  rmse  2.74  max  9.55
```

段0 自己就有 17.75mm 的尖峰, 与切段无关 ⇒ **尖峰是真的算法误差, 不是对齐折中**。

### 16.5 结论

**两个问题都有, 但分工是清楚的:**

1. **数据/真值侧的问题**(占新数据的**大头**): 09-16 基站被碰撞后重标的 v11 几何下,
   tracker 的**动态**解不稳定 —— v11 会话 **12/12** 出现中途分支跳变, 而 v10 是 2/13。
   注意 v11 的**静止**验收很漂亮(位置 P95/最大 0.231/0.311 mm, 独立进程重启差 0.034 mm)却
   完全没拦住这件事, 因为静止测试只测抖动、对这类分支不稳定是盲的。
   **后果**: 09-16 之后的**任何** take 都没有可信真值, 新录制的精度数字目前**测不出来**。
2. **算法侧的问题**(占老数据的**全部**, 且在新数据里依然存在): 剥离真值损伤后,
   **尖峰(15.9~20.6mm)与姿态(1.9~16.7°)依然超门**。这与 §3~§14 的结论完全一致 ——
   尾段 19 个配置都动不了它, 瓶颈在上游(VINS 度量尺度 + 输入质量门)。

**对本报告既有结论的影响: 没有。** §3~§14 的 18 项全部来自 v10 代的会话, 其中 11/13 干净;
唯一受污染的是 `batch5/group1`(§16.3 量化: 真值损伤只值 +2.3mm / 19.3mm 的 13%),
**且剥掉之后判定不变**。所以 §3~§14 的达标数(1)与 REJECT 数(9)都站得住。

**下一步的优先级建议**:
1. **先修真值链** —— v11 几何下的动态分支不稳定必须先解决, 否则之后所有录制都白跑。
   最小验证: 拿一条 loop1 take 的原始 lighthouse 数据, 用 v10 / v11 两份配置各离线复算一遍
   tracker, 看分支跳变是否随配置出现/消失(需要 `lighthouse_raw.rec.gz` 级别的原始数据)。
2. 真值链修好之前, **不要再录新 take**; 也不要基于 §15 那三个 FAIL 数字做任何判断。
3. 算法侧维持 §14 的结论: 尾段已扫穿, 要动就得动上游。

## 17. tracker 真值链为什么跳变 —— 机制已定位

§16 把跳变定位到标定代(v11 代 12/12 跳, v10 代 2/13), 但没查到机制。本文档此前写的
下一步是"拿原始 lighthouse 数据用 v10/v11 两份配置离线复算"—— **那条路走不通**:
会话目录(`reports/lighthouse_umi_sessions/<会话>/`)里只有 `tracker.csv`、
`libsurvive_runtime_config.json`、`tracker_integrity.json`、`tracker.log`, **没有 sweep 级的
原始录制**, 无法离线重放解算。改用两条不依赖原始数据的路径, 机制已经问出来了。

### 17.1 标定代之间只改了基站位姿, 几何没有退化

把全部 51 个会话的 `libsurvive_runtime_config.json` 解析出来逐项对比
(`poser=MPFIT`、`disambiguator=StateBased`、两站 id、mode 3/2 全程不变):
变的只有 `lighthouse0/1` 的 `pose` 与 `floor-offset`。位姿指纹清晰地分成三代:

| 代 | 会话时段 | LH0 位姿指纹 |
|---|---|---|
| — | 09-12 / 09-13 | `75a9533c` |
| v10 | 09-14 / 09-15 | `09069581` |
| v11 | 09-16 起 | `58dc4932` |

v10 → v11 是 LH0 位移 23cm、LH1 位移 9cm —— **就是"被撞"那次重标定**。几何本身:

| | 基线 | 两站朝向夹角 | 从原点看张角 |
|---|---|---|---|
| v10 | 1762.2 mm | 176.92° | 125.03° |
| v11 | 1748.2 mm | 175.15° | 129.17° |

两代都是近反平行配置, **没有退化**。所以"v11 把基站标成了病态几何"这个假说**排除**。

另: 全库唯一 `unlock_count != 0` 的会话是 `20260917_232851_720p_loop1`(LH0 = 83), 但它的
两站位姿与同时段其它会话**一致到 1e-5**(差异只是浮点尾数)。`unlock_count` 是个计数器,
不是漂移 —— 与 §16.2 我那次误判同源, 别再被它带偏。

### 17.2 IMU 裁判: 试过了, 在这个速率区间分不开

思路: `tracker.csv` 的姿态是 world←tracker, 相邻样本的相对旋转与同区间陀螺积分在同一个
体系里, 可以直接比; 陀螺是独立于 Lighthouse 的物理传感器, 看似理想裁判。

去时间偏置后确实很漂亮 —— **全序列中位残差只有 0.024°(235329)/ 0.011°(233028)**,
说明 tracker 平时姿态解得极准。但**在跳变处失效**:

- 跳变全部落在 tracker 自身 500–1500 deg/s 的快转段。在 1500 deg/s 下, **±2ms 的时间偏置
  误差就是 ±3°**, 与待判的 6° 同量级。235329 那次跳变的 IMU 读数随偏置选择在
  0.055°→4.178° 之间摆动 —— 判据不稳定。
- 更糟的是 **IMU 自己在这个区间吐垃圾**: 残差最大的 12 个样本, IMU 全部报 ~3300 deg/s
  而 tracker 只报 ~40 deg/s(近似饱和的固定值)。用这种数据当裁判会把 IMU 的故障算成
  tracker 的故障。

**结论: IMU 不能用来自动判定 tracker 跳变。** 我一度据它下了"跳变不是真运动"的结论,
那个结论本身很可能仍成立, 但**当时的证据强度不够, 已撤回**; 下面 17.3 是不依赖 IMU 的判据。

### 17.3 稳健判据: 跳变全部挤在 tracker 自身转速的最顶端

只用 `tracker.csv`。对每个被门检出的跳变, 算该样本的角速率 `dR/dt`, 看它落在本条 take
角速率分布的什么分位上(`tracker_jump_rate_audit.py`):

- **56 条会话, 21 条有跳变, 共 96 次跳变**
- 跳变处角速率分位: **中位 99.6%**; **88/96 落在 90 分位以上**; **95/96 落在 50 分位以上**
- 各条 take 的角速率 P95 只有 ~30 deg/s, 而跳变发生在 **46–3751 deg/s**

**机制: 跳变全部发生在 tracker 自身转速最高的瞬间。** 正常转速下解算稳定, 一旦转速冲到
分布顶端, Lighthouse 解算给出的位姿就跳到另一支自洽解上。这不是随机的分支歧义, 是
**快转下的解算失效**。

可能的下游原因(未逐一证实, 按嫌疑排序): 快转时设备扫过基站视场的时间变短、命中 sweep
变少, 解算欠约束; 且 gen2 两站是时分复用的, `disambiguator: StateBased` 的
`PropagateState` 明说**允许跳状态**("can skip states if its been a while since its been
able to process"), 而 `LighthouseState_findByOffset` 取**最近**的状态槽 —— 一个时序偏掉的
脉冲会被归给相邻的站/轴, 解出来的就是自洽但错的位姿。

### 17.4 v10 与 v11 的差别是幅度, 不是频率

按代看各条 take 的 `max_translation_step_mm`:

- **v10 代(09-14/09-15)**: 2.82, 2.98, 4.06, 4.18, 4.26, 4.30, 4.31, 4.53, 4.89, 5.75, 5.87 mm
- **v11 代(09-16 起)**: 8.90, 8.90, 9.18, 9.33, 10.01, 10.91, 13.03, 15.61, 16.92, 19.60 mm

两代之间在 **6–9mm 处一刀切开**, 没有任何重叠。跳变在两代都发生(见 17.3), 差别是
**v11 几何下同样的失效造成的偏移大了一倍以上**, 于是越过了门的阈值, 才表现为"v11 代
12/12 REJECT"。这解释了 §16 观察到的代际断层。

### 17.5 现有的两道门都漏

- `tracker_integrity.json` 的 `translation_pose_jump_count` **严重漏报**:
  `235329` 报 1 次, 实际 8 次; `233028` 报 0 次(`status: PASS`), 实际 2 次且含一次 13.03mm。
- `lighthouse_tracker_branch_gate.py`(3mm 绝对下限 + P95×3)比 integrity 好, 但仍只覆盖
  幅度大的那些。
- 本目录新增 `tracker_jump_rate_audit.py`, 用 17.3 的角速率分位做前置过滤 —— 它**不需要
  IMU、不需要原始数据**, 只读 `tracker.csv`, 可以作为录完就跑的即时体检。

**给重标定的直接建议**: 基站重标之后, 先拿一次**含快转**的 take 跑
`tracker_jump_rate_audit.py` 看跳变幅度是否回到 v10 代的 3–6mm 量级; 只有回到那个量级,
真值链才算可用。静止验收(§ v11 的 P95 0.231mm)**测不出这个问题** —— 它只测抖动, 对快转下的
解算失效是盲的。
