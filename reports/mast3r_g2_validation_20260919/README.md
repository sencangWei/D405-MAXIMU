...
### 17.8 跳变倾向是**标定代本身的属性** —— 但"代"必须用数值判定, 不能用哈希

§17.5 用的是"每条 take 的最大单步", 那受运动量影响。按**行进距离**归一之后:

| 代 | 会话 | 跳变 | 总路程 | **每米跳变** | 该代静止验收 位置 P95 |
|---|---|---|---|---|---|
| v8 | 9 | 28 | 48.17 m | 0.581 | —（未找到） |
| v9 | 12 | 14 | 44.90 m | 0.312 | **0.0238 mm** |
| **v10** | 13 | **2** | **79.42 m** | **0.025** | 0.0568 mm |
| **v11** | 12 | **50** | 45.80 m | **1.092** | 0.2310 mm |

**v10 与 v11 相差 44 倍, 而 v10 总路程还更长** ⇒ 不是"v11 动得多"。
**跳变倾向确实是标定代的属性, 且 v10 证明了 0.025 次/米是做得到的。**

> **⚠ 分组本身有个坑, 记在这里免得再踩。** 我最初用"基站位姿字符串的 sha256"分组,
> 得到 6 组。用**数值差**重判后发现: `69c93307` 与 `fc90e4b7` **对 v8 冻结配置都是
> 0.00mm/0.00°**(同一代), `8e2fc3eb` 与 `dccd24e7` **对 v11 都是 0.00mm/0.00°**(同一代)。
> 哈希把**同一个标定**按浮点尾数劈成了两组 —— 与 §17.1 那个 `unlock_count` 假警报
> **是同一个坑**(1e-5 的尾数差)。**判标定代必须比数值, 不能比哈希。**

### 17.8.1 决定性反例: 静止验收噪声**预测不了**跳变率

§18 的初稿据此推荐"用静止 P95 当验收阈值"。**这个推论被 v9 证伪了**:

| 代 | 静止 P95 | 每米跳变 |
|---|---|---|
| v9 | **0.0238 mm**(最好) | 0.312 |
| v10 | 0.0568 mm | **0.025**(最好) |
| v11 | 0.2310 mm | 1.092 |

**v9 的静止噪声比 v10 好 2.4 倍, 跳变率却比 v10 差 12.5 倍。** 单调性不成立。

同理, `variance`(§17.9)在 4 个代上也不单调: v8 的 LH0 方差(2.04e-3)比 v11(5.85e-4)大 3.5 倍,
跳变率却更低(0.581 vs 1.092)。**§17.9 那条线索到此为止, 不作为验收指标。**

⇒ **静止验收(无论看 P95 还是 variance)对运动下的跳变率没有预测力。**
唯一可靠的验收是**跑一次含运动的 take**, 直接量每米跳变率。

### 17.8.1b 另一个被证伪的假说: GSS 的 auto-floor-height

配置里 `floor-offset` 在 v10→v11 变了 **21.6mm**(−0.1720 → −0.1504), 与跳变量级相符,
而且源码看上去完全对症 —— `driver_global_scene_solver.c`:

```c
if(gss->auto_floor) {                      // gss-auto-floor-height 默认 = 1 (开启)
    FLT min_z = gss->ctx->floor_offset;    // 从**当前**值起
    for (...) min_z = linmath_min(min_z, gss->scenes[i].pose.Pos[2]);
    survive_set_floor_offset(gss->ctx, min_z);   // ⇒ 只减不增
}
```

而输出是 `p.Pos[2] -= ctx->floor_offset`。**floor_offset 每下调一次, 之后所有点的 Z
就整体上抬一个常量** —— 纯 Z、+Z、不回位、与水平运动无关, 字面上符合全部观测特征。
日志也确认加载了 `GlobalSceneSolver`。

**但实测证伪**: 跳变向量的 `|dz| / |d|` 中位只有 **0.179**(纯 Z 应为 1.0), **89/96 小于 0.5**;
dz 符号混杂(**+Z 65 次 / −Z 31 次**), 不是单调下调该有的样子。**排除。**

**不过这条测试捞出一个新事实**: **跳变以水平(XY)方向为主**, `|dz|/|d|` 中位 0.18。
这条加进 §17.4 的刻画里 —— 是水平面内的位移错位。

### 17.8.1c 跳变前后有没有"解算退化期"? 没有(短期内)

**动机**: 台阶型位移的另一种解释是"锁丢了一小段, 解算在新解附近重新收敛", 那样
跳变前后应能看到残差先涨后落(过冲/回敛), 而不是一步到位。

**做法**: 对每次跳变, 用跳变前 10 点的速度外推, **再加上跳变向量**, 看跳变后第 m 个
采样点与该外推之差; 对照取**跳变前速度相近(±25% 或 ±2cm/s)**的普通样本, 逐样本相减。

| m (采样点) | 跳变处残差 − 配速对照 (mm) 中位 | P25 | P75 | >1mm 占比 |
|---|---|---|---|---|
| **1** | **0.45** | 0.17 | 1.09 | 27% |
| 5 | 2.08 | 1.15 | 4.64 | 76% |
| 20 | 5.35 | 1.44 | 14.40 | 81% |
| 40 | 9.19 | 1.37 | 20.02 | 76% |

**短期结论(m=1, 即 8ms)**: 只差 0.45mm、73% 在 1mm 内 ⇒ 跨过跳变那一刻, 轨迹与
"原轨迹 + 跳变向量"吻合到亚毫米, **没有过冲、没有回敛、没有退化期** —— 是一个
干净的瞬时台阶。**排除"锁丢失后重新收敛"这一类机制。**

> ⚠ **m ≥ 5 的数字不可用, 别引用。** 机械臂本身在加速, 匀速外推的残差随 m 自然上涨;
> 配速只能配"速度", 配不了"当时那一刻的加速度", 所以长时程上对照组和跳变组不是
> 同一回事(中位 9.19mm 里绝大部分是这个混淆)。这里只采信 m=1。

### 17.8.2 已排除的机制(累积)

| 假说 | 判据 | 结果 |
|---|---|---|
| 快转打崩解算 | 剔除跳变本身后看前后邻步速率 | 只到 ~73–81 分位, 中位 17.8 deg/s ⇒ 排除（§17.3） |
| 时间错位/陈旧解 | `位移/速度` 应聚在采样周期 8ms | 0/96 落在 5–15ms ⇒ 排除 |
| 运动方向拖尾 | 纯平移跳方向应与运动同向 | cos 中位 −0.25 ⇒ 排除 |
| 离散分支翻转 | 跳变应含大旋转 | 旋转中位 0.91°, 无一超 30° ⇒ 排除（§17.4） |
| 基站几何退化 | 基线/朝向夹角/张角 | 各代均近反平行 ⇒ 排除（§17.1） |
| GDOP 病态 | 法方程条件数 | v10 3.63 vs v11 3.93 只差 8%, 对应 44 倍差 ⇒ 排除 |
| 出厂角度标定被改 | `fcal*` 逐字段 diff | 各代全同 ⇒ 排除（§17.9） |
| 静止噪声大 | 静止验收 P95 | v9 反例 ⇒ **排除**（§17.8.1） |
| 标定方差大 | 记录的 `variance` | 4 代上不单调 ⇒ **排除**（§17.9） |
| GSS 自动 floor 下调 | 跳变应纯沿 Z | `\|dz\|/\|d\|` 中位 0.179, 89/96 < 0.5 ⇒ **排除**（§17.8.1b） |
| 锁丢失后回敛(退化期) | 跨跳变那步应与"原轨迹+跳变"差 ≈ 0 | 配速对照后 m=1 只差 0.45mm ⇒ **排除**（§17.8.1c） |

即: 触发条件是**标定结果里某个还没被测到的量**。

---

## 18. 给下一次重标定的验收建议

基站昨晚又被移动了 ⇒ 需要 v12。§17 能转成的**唯一**可靠建议是下面这条:

### 18.1 唯一可靠的验收: 跑一次含运动的 take, 量每米跳变率

**静止验收测不出这个问题** —— 这是 v9 与 v10 的对照教给我们的硬结论(§17.8.1):
v9 静止 P95 只有 0.0238mm(历代最好), 跳变率却是 0.312/m(比 v10 差 12.5 倍)。
**"静止很稳"与"运动时不跳"是两件事, 前者不蕴含后者。**

所以重标定后必须:

1. 录一条**含正常运动**(不需要刻意快转 —— §17.3 已排除快转)的 take, 覆盖实际工作包络。
2. 跑 `lighthouse_tracker_branch_gate.py` 取跳变列表。
3. 跑 `tracker_jump_rate_audit.py` 或直接算 **跳变数 / 总路程**。
4. **判据: 每米跳变 ≲ 0.05**(v10 是 0.025; v9 是 0.312, v11 是 1.092)。
   落在 0.3 以上就别开始录正式数据。

### 18.2 标定输入量: 对齐 v10

| | v10（0.025/m） | v11（1.092/m） |
|---|---|---|
| 标定输入 | **32 个稳定场景 / 1024 组测量**（3 段原始记录 run7+run8+run10 联合求解） | **13 有效场景 / 最大测量数 436** |

v11 只用了一半不到的观测。建议至少 3 段原始记录、目标 ≥1000 组测量。
（⚠ 这是**基于两代的观察**, 不是已验证的因果。）

### 18.3 明确**不要**用的两条判据

- **静止位置 P95** —— 别看它下结论。v11 的 0.231mm 当时没被当成问题是对的,
  但 v9 的 0.0238mm 同样不能当成"标定很好"的证据。
- **配置里的 `variance`** —— MPFIT 根本不消费它(消费它的是 kalman lighthouses,
  我们 `poser: MPFIT`), 且 4 代上不单调。可以记录备查, 不要当门。

### 18.4 仍然未知的

**触发条件还没找到**(§17.8.2 排除了 9 条假说)。所以上面 18.1 是**经验判据**(直接量结果),
不是机制性判据 —— 它能在事后判"这次标定行不行", 但**不能预告下一次会不会好**。
要把这件事真正解决, 需要**逐样本的解算器诊断**(种子是否重启 / 观测到几站几条 sweep /
残差多少), 那是采集侧的改动。这一点在重标定之前值得先决定做不做。
