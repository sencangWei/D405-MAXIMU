...
### ★★ 结论：**两把尺子会指向不同的模式** ⇒ 只看 RMSE 会挑错

| cell（tight） | joint（现役） | stereo | imu | no_g7 | RMSE 最优 | MAX 最优 | |
|---|---|---|---|---|---|---|---|
| v10b/g1 | **2.62** / 13.5 | 2.67 / 13.9 | 2.81 / **13.1** | 3.58 / **10.6** | joint | imu | 冲突 |
| v10b/g2 | 7.75 / **16.8** | **6.01** / 23.0 | 11.44 / 23.8 | 14.78 / 45.6 | stereo | joint | ★**冲突** |
| v11b3/g2 | 4.67 / 10.6 | **4.16** / **10.2** | 5.47 / 13.1 | 7.92 / 22.8 | stereo | stereo | 一致 |
| batch5/g1 | 5.38 / 19.2 | 5.47 / 25.4 | **4.81** / **14.7** | 10.70 / 41.4 | imu | imu | 一致 |
| batch5/g2 | 4.65 / 15.0 | **3.92** / **12.7** | 5.55 / 17.4 | 8.75 / 32.6 | stereo | stereo | 一致 |
| batch5/g3 | 7.66 / 19.3 | **6.54** / 21.5 | 9.38 / **18.8** | 12.37 / 37.4 | stereo | imu | 冲突 |
| batch5/g4 | **4.75** / 11.4 | 5.03 / 13.7 | 4.83 / **10.1** | 6.89 / 16.5 | joint | imu | 冲突 |

（每格 `RMSE mm / MAX mm`；`nan` 的 2 条见下方说明，不计入）

### ★★ 结论：**两把尺子在 4/7 条 cell 上选出不同的模式** ⇒ 只看 RMSE 会挑错

**可量化的判决**：在 {`joint`, `stereo`, `imu`} 三者里，
**RMSE 最优的模式 ≠ MAX 最优的模式** 出现在 **4/7** 条（仅 3/7 一致）。
最刺眼的是 v10b/g2：`stereo` 把 RMSE **7.75 → 6.01（−22%）** 同时把
**MAX 16.8 → 23.0（+37%）**，而**门查的是 MAX**。

⇒ **「只报 RMSE 的消融会指向错的方向」这件事被证明**（第一版消融正是如此，
留档 `g7_ablation_rmse_only_v1.log`）。

**⚠ 但不要过度概括成「stereo 一律有害」** —— 它在 v11b3/g2 与 batch5/g2 上两把尺子都赢。
**立得住的判决理由是「胜负在 cell 之间不一致、且两把尺子会冲突」，
不是「某个模式更差」。**
...
* **`joint`（现役）从不在 MAX 上最差**；MAX 最优在 imu / joint / stereo 上各出现过。
* **`no_g7`（跳过整个 `[7/8]`）在 v10b/g1 上 MAX 最好（10.6，几乎过门）却 RMSE 最差。**
  ⇒ **「跳过 `[7/8]`」有机会救 max**，代价是 RMSE 全面变差（与 §5
  「`[7/8]` 整体有益、局部放大峰」一致）。**值得继续查：有没有「保留 RMS 收益、
  又不抬峰」的中间档**（例如更弱的图优化、或按鼓包窗自适应地放松修正）。
