...
| cell（tight） | joint（现役） | stereo | imu | no_g7 | 逐 cell 最优 |
|---|---|---|---|---|---|
| v10b/g1 | **2.62** / 13.5 | 2.67 / 13.9 | 2.81 / **13.1** | 3.58 / **10.6** | RMSE=joint，MAX=imu（g7 内） |
| v10b/g2 | 7.75 / **16.8** | **6.01** / 23.0 | 11.44 / 23.8 | 14.78 / 45.6 | **RMSE=stereo 而 MAX=joint ⇒ 反向** |
| v11b3/g2 | 4.67 / 10.6 | **4.16** / **10.2** | 5.47 / 13.1 | 7.92 / 22.8 | 两者都 stereo |
| batch5/g1 | 5.38 / 19.2 | 5.47 / 25.4 | **4.81** / **14.7** | 10.70 / 41.4 | 两者都 imu |
| batch5/g2 | 4.65 / 15.0 | **3.92** / **12.7** | 5.55 / 17.4 | 8.75 / 32.6 | 两者都 stereo |

（每格 `RMSE mm / MAX mm`）

**准确的说法（不要过度概括）**：

* **至少有一条 cell（v10b/g2）两把尺子方向完全相反** —— `stereo` 把 RMSE
  **7.75 → 6.01（−22%）** 同时把 **MAX 16.8 → 23.0（+37%）**。
  ⇒ **「只报 RMSE 的消融会指向错的方向」这件事是被证明了的**，
  第一版消融正是如此（留档 `g7_ablation_rmse_only_v1.log`）。
* **但并非处处相反**：`batch5/g1` 是 imu 两把尺子都赢，`batch5/g2` 与 `v11b3/g2`
  是 stereo 两把尺子都赢。⇒ **不存在一个跨 cell 一致最优的模式。**
* **`joint`（现役）从不在 MAX 上最差**（5 条里 MAX 最优出现在 imu/joint/stereo，各不同）。
* `no_g7` 在 v10b/g1 上 MAX 最好（**10.6**，几乎过门）却 RMSE 最差
  ⇒ 「跳过 `[7/8]`」**有机会救 max**，代价是 RMSE 全面变差（与 §5
  「`[7/8]` 整体有益、局部放大峰」一致）。**这是值得继续查的线索，不是本轮结论。**

**⇒ 判决：不据此改生产参数。** 理由不是「stereo 有害」，而是
**胜负在 cell 之间不一致、且两把尺子会冲突** —— 与用户明令一致
（「不要为单一轨迹改 `--metric-scale-mode`；任何更改必须是在**所有数据组**上
验证过的通用策略，且由用户决定」）。注意 `stereo` 还会**跳过
`imu_stereo_metric_scale_disagreement` 硬门**（`fuse_mast3r_stereo_imu.py:2770-2775`）
—— 那是 Codex 2026-09-11 定过、只接进 `compare)` 而从没接进产品路径的策略。
**所以若哪天要动它，不能只看精度，得同时决定那道尺度硬门要不要一起放开。**
